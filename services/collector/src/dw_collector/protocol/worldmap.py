...
A viewport response carries `points`, a list of strings shaped
`b64:<base64>`. The key is the constant `b64` — it names the encoding — on
all 7,678 entries of the sweep capture; the value is a protobuf with no
...
# The one key every point string carries. Kept as a constant rather than
# ignored: a second key would mean a second message shape, and silently
# decoding it as this one is how a payload becomes plausible nonsense.
POINT_KEY = "b64"
...
def decode_point(entry: str) -> Tile:
    ...
    """
    key, _, blob = entry.partition(":")
    if not blob:
        msg = "point is not '<key>:<base64>'"
        raise WorldMapDecodeError(msg)
    if key != POINT_KEY:
        msg = f"unexpected point key {key!r}, refusing to decode as a tile"
        raise WorldMapDecodeError(msg)
    try:
        raw = base64.b64decode(blob, validate=True)
    except (binascii.Error, ValueError) as exc:
        msg = f"point is not valid base64: {exc}"
        raise WorldMapDecodeError(msg) from exc
