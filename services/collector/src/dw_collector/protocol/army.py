...
and maxLv. That is what pins the identity and level fields.
    ...
    2.2      the hero's OWN level. Not what the screen shows when 2.14 is
             present — see below
    ...
    2.14     the level a training-centre hero is synced to, and therefore the
             level the game displays for it. Guaplee's lineup pinned this:
             the screen shows 120 for all five, 2.2 reads 120/90/1/70/1, and
             exactly the four synced heroes carry 2.14 = 120. Reading 2.2
             alone reports level 1 for a hero the game shows at 120
...
class ArmyUnit:
    ...
    #: What the game displays: the synced level when the hero sits in the
    #: training centre, its own level otherwise.
    level: int | None = None
    #: The hero's own level. Differs from `level` only for a synced hero, and
    #: is frequently 1 for one that has never been levelled directly.
    base_level: int | None = None
    #: Whether `level` came from the training centre rather than the hero.
