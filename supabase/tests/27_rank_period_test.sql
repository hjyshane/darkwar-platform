...
select is(public.rank_period_start('2026-07-27T02:00:00Z'), '2026-07-27T02:00:00Z'::timestamptz,
  'the epoch is its own period start');
select is(public.rank_period_start('2026-07-27T01:59:59Z'), '2026-07-13T02:00:00Z'::timestamptz,
  'one second before the epoch falls in the previous period');
select is(public.rank_period_start('2026-08-03T02:00:00Z'), '2026-07-27T02:00:00Z'::timestamptz,
  'the odd week in between belongs to the period that started before it');
select is(public.rank_period_start('2026-08-10T01:59:59Z'), '2026-07-27T02:00:00Z'::timestamptz,
  'and so does the last moment of it');
select is(public.rank_period_start('2026-08-10T02:00:00Z'), '2026-08-10T02:00:00Z'::timestamptz,
  'two weeks on is the next period');
select is(public.rank_period_start('2026-08-05T13:45:12Z'), '2026-07-27T02:00:00Z'::timestamptz,
  'midweek lands on the period it is inside');
select is(public.rank_period_start('2026-01-05T02:00:00Z'), '2025-12-29T02:00:00Z'::timestamptz,
  'well before the epoch still resolves, on the same fortnightly grid');
select is(public.rank_period_start('2027-07-26T02:00:00Z'), '2027-07-26T02:00:00Z'::timestamptz,
  'a year on stays on the grid');

-- The measurement points. 01:59, not 02:00: a minute later the game has
-- wiped the weekly boards and everybody scores zero.
select is(public.rank_period_week_ends('2026-07-27T02:00:00Z'),
  array['2026-08-03T01:59:00Z'::timestamptz, '2026-08-10T01:59:00Z'::timestamptz],
  'the weekly readings land a minute before the boards clear');

select is(public.rank_period_start((public.rank_period_week_ends('2026-07-27T02:00:00Z'))[1]),
  '2026-07-27T02:00:00Z'::timestamptz,
  'and the first one is inside the period it belongs to');
select is(public.rank_period_start((public.rank_period_week_ends('2026-07-27T02:00:00Z'))[2]),
  '2026-07-27T02:00:00Z'::timestamptz,
