-- 0015: contribution snapshots project into the players summary, newer wins,
...
select is((select daily_donation_score from public.players
           where game_uid = 58011111000580), 5860::bigint,
  'daily donation lands in the summary');
select is((select daily_donation_updated_at from public.players
           where game_uid = 58011111000580), '2026-07-30T04:38:11Z'::timestamptz,
...
select is((select alliance_battle_score from public.players
           where game_uid = 58011111000580), 42000::bigint,
...
select has_column('public', 'players', 'alliance_battle_updated_at',
