-- 0072: officers are measured, just not ranked.
--
-- 0071 took R4 and above out of the pool AND stopped computing their figures.
-- Only the first half was wanted. An officer's donation total is a fact worth
-- looking at — it is often the largest in the alliance — and blanking it means
-- the screen cannot answer "how much did the leadership actually give". What
-- must not happen is their total bending everybody else's percentile, or a tier
-- being handed to somebody who is not competing for one.
--
-- So two different exclusions, which 0071 had conflated:
--
--   NOT RANKED (R4, R5). Every figure computed, including a percentile and an
--     activity score, measured AGAINST the ranked pool without joining it. No
--     tier: nothing is being decided about them.
--
--   NOT MEASURED (joined within the last two weeks). Nothing computed. They
--     cannot have a fortnight's contribution, so a percentile against people
--     who could is not a fact about them at all — it is an artefact of when
--     they joined. This is the exclusion that was asked for as "exclude from
--     the calculation".
--
-- The percentile for an unranked officer is "what fraction of the ranked pool
-- did they beat", computed against the pool rather than with it. Same question
-- percent_rank answers for a member of the pool, asked from outside.
...
begin
  ...
  select
    p_period_start, player_id, game_uid, current_name, 3,
    donation_week1, donation_week1_at, donation_week2, donation_week2_at,
    duel_week1, duel_week1_at, duel_week2, duel_week2_at,
    donation_total, duel_total,
    power_start, power_start_at, power_end, power_end_at, power_growth,
