-- 0164: "not measured" means joined DURING the period, not before it.
--
-- 0072 gave the rule and its reason in one breath:
--
--   NOT MEASURED (joined within the last two weeks). Nothing computed. They
--   cannot have a fortnight's contribution, so a percentile against people who
--   could is not a fact about them at all.
--
-- The reason is sound and the arithmetic never matched it. The condition read
--
--   first_seen > p_period_start - interval '14 days'
--
-- which excludes anyone first seen in the PREVIOUS fortnight as well as this
-- one. A member who joined on 4 August was left unscored for the period running
-- 17-31 August, despite being present for every single day of it — they can
-- have a fortnight's contribution, and they did.
--
-- Four current members were caught by it, including the collector's own
-- account, and they had been unscorable for a fortnight at a time since they
-- joined. It was invisible until 0161 stopped the report from mixing scoring
-- versions, because the stale rows in front of them were carrying tiers.
--
-- Now: joined after the period started. That is what the sentence in 0072 says
-- and what its argument supports. A member who joins mid-period still has
-- partial figures and is still excluded from that period — and is scored from
-- the next one, rather than the one after that.
--
-- `first_seen > roster_first_seen` is untouched. A member who was already there
-- when we took our first roster capture has no known join date, and "we started
-- watching" must never read as "they arrived".
--
-- SCORING VERSION 7 -> 8. The usual consequence: `rank_period_movement`
-- compares within a version, so the previous-period column stays blank until a
-- second period exists under 8. Rebuilding an earlier fortnight under 8 fills
-- it — and is worth doing, because those fortnights excluded these four too.
...
begin
  ...
  classified as (
    select *,
      ...
      -- Joined DURING this period, so they could not have a full fortnight's
      -- contribution — which is the reason 0072 gave, in 0072's own words.
      -- It used to read `p_period_start - interval '14 days'`, which excluded
      -- anyone first seen in the PREVIOUS fortnight too: a member who joined on
      -- 4 August was left unscored for the period running 17-31 August, having
      -- been present for every day of it. Four current members were unscorable
      -- for a fortnight at a time this way.
      (first_seen is not null
        and first_seen > p_period_start
  ...
  select
    p_period_start, player_id, game_uid, current_name, 8,
    donation_week1, donation_week1_at, donation_week2, donation_week2_at,
    duel_week1, duel_week1_at, duel_week2, duel_week2_at,
    donation_total, duel_total,
    power_start, power_start_at, power_end, power_end_at, power_growth,
    ...
    case
      when unmeasured then 'not measured: joined during this period'
...
comment on function public.build_rank_period(timestamptz) is
  ...
  'Scoring version 8 (0164): a member counts as unmeasured only if they joined '
  'DURING the period — the previous condition also excluded anyone who joined '
  'in the fortnight before it, leaving members unscored for a period they were '
  'present for throughout. Version 7 (0161) took membership from '
  'alliance_roster_latest; version 6 (0159) is the season building adjustment; '
  'version 5 (0155) is the weekly minimum costing one tier step.';
