...
| `arena_entries` | 800 | 아레나 순위·점수·**방어 전투력** |
| `arena_entry_heroes` | 3,998 | **아레나 방어 편성 전체** — 영웅·성급·장비·스킬 |
...
**아레나 방어 편성이 공개된다는 뜻이다.** 어떤 영웅을, 몇 성으로, 어떤 장비로
세워 뒀는지가 주소만 알면 누구에게나 읽힌다. 상대 연맹 입장에서는 정찰 자료다.
게임 안에서도 순위표를 열면 볼 수 있는 정보이긴 하지만, **한 번에 3,998행을
긁어갈 수 있는 형태로 놓는 것은 다른 이야기다.**

선택지는 셋이다:

1. **그대로 공개** — 게임 안에서도 보이는 정보라고 판단하면 이대로 간다.
2. **아레나만 member 전용으로 돌린다** — 마이그레이션 하나면 된다.
   `arena_entries` / `arena_entry_heroes`의 `public_read` 정책을 member 정책으로
   바꾸고, §20.2대로 pgTAP 부정 테스트를 붙인다. 대시보드의 아레나 탭은
   `members.view`와 같은 방식으로 가린다.
3. **전부 로그인 뒤로** — `players`까지 막으면 크로스서버 랭킹·서버 페이지·
   아레나 이름 표시가 전부 로그아웃 상태에서 빈다. 사실상 "공개 대시보드"를
   포기하는 선택이다.
