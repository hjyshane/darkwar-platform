...
| `al.battle.rank.info` | **승격** | `{uid, name, score}`. `type` 0=일간, 1=주간 (2026-08-01 라벨 캡처로 확정). `type=2`는 인원이 다르고(94 vs 165) 라벨이 없어 미확정. 파서는 2026-07-30에 작성됐으나 푸시되지 않아 다른 머신에서 보이지 않았고, 그 사이 두 문서가 "미승격"으로 잘못 적었다. 2026-08-01 리베이스 후 머지. `variant` 해석은 캡처 백로그 0번이 계속 막고 있다 |
