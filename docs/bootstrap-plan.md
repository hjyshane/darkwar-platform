...
>   vitest 36 · pgTAP 78.
> - **S15 준비 완료(하드웨어 불필요분)** — 캡처 엔진(`capture/session.py`)은
>   TCP 세그먼트만 받고 출처를 모른다 → scapy 소스(`capture/live.py`)는 얇은
>   어댑터이고, 재조립·아웃바운드 무시·미확인 발견·불량 payload 계수·중복 흡수를
>   전부 손으로 만든 패킷으로 테스트. `dw-capture`는 실제 엔트리포인트가 됨.
> - **ADB 가드(FR-COL-001/010, FR-OPS-006)** — 레거시엔 denylist가 **없고**
>   시리얼 미설정 시 `devices[0]`를 자동 선택한다(본계정을 집을 수 있는 경로).
>   포팅이 아니라 그 공백을 메우는 코드: 자동 탐지 거부, 설정 누락 거부,
>   지정된 수집기 시리얼 외 전부 거부, 킬스위치 파일 지원.
> - **하트비트(FR-COL-007)** — 상태를 주장하지 않고 저널에서 유도(침묵→degraded,
>   데드레터가 얕은 큐보다 우선). sync 루프가 매 비트 보고하므로 보낼 게 없어도
>   멈춘 수집기가 보인다. pytest 113.
> - **다음(차단됨)** — S15 실행(네이티브 Windows + Npcap에서 `uv sync --extra
>   capture` 후 `dw-capture`), P6/S13 Discord 런타임(앱 등록·터널·OAuth 시크릿
>   필요). 하드웨어·외부 계정 없이 할 수 있는 일은 소진.
