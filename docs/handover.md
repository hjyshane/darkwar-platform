...
| 8 | 아레나 영웅 정보 | 완료 — 편성·병종·전용무기·장비·스킬·병사수 (0025). `stage`는 판정 완료(0-b)이나 **아직 저장되지 않는다** — 디코더만 내보내고 컬럼이 없다. 영웅 이름은 카탈로그(0037)에서 온다 |
...
**미해석 필드는 남아 있지 않다.** `2.14`는 훈련소 레벨(0026), `2.9`는 승급 단계
`stage`로 확정됐다(0-b, 2026-08-02). 다만 **`stage`는 아직 컬럼이 없다** —
디코더가 `ArmyUnit.stage`로 내보내지만 `arena.py`가 읽지 않아 관측마다 버려진다.
판정과 승격은 다른 일이고, 승격은 지금 안 되어 있다.
