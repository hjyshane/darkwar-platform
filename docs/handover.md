...
- **0-b번** — 질문이 **하나로 줄었다.** `2.14`는 훈련소 레벨로 이미 해결됐고
  (0026), `2.9`는 `push.hero.data`의 `stage`와 대조해 **이름을 알아내 디코더가
  `stage`로 내보낸다** — payload 성급 6(=**게임 5성, 최고**)에서 1,973/1,973이
  0이고 그 아래에서 0~4다. 남은 건 그게 **화면에 어떻게 보이는가** 하나뿐이라
  30초짜리다. 게임 기준 **4성 이하** 영웅이 한 기 필요하다(그래서 지난 캡처가
  이걸 못 닫았다 — 대상이 전부 5성이었다).
...
| 8 | 아레나 영웅 정보 | 완료 — 편성·병종·전용무기·장비·스킬·병사수 (0025) + `stage`. 남은 것은 `stage`의 화면 표시 확인뿐(백로그 0-b) |
