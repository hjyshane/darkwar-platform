...
> **다음 세션은 바로 아래 「2026-09-09 상태 (11)」부터 읽는다.** 그 아래는 배경이고,
> 일부는 이미 낡았다.
...
- **타입 생성이 `--local`과 `--linked`로 갈린다.** CI는 `--local`이라
  `__InternalSupabase` 블록이 없어야 하고, 저장소 스크립트(`pnpm db:types`)는
  `--linked`다. 다음 `--linked` 재생성이 `db` 잡을 다시 깨뜨린다. **아직 정하지
  않았다** — 스크립트를 `--local`로 바꾸든 CI를 `--linked`로 바꾸든 한쪽으로
  통일해야 한다.
