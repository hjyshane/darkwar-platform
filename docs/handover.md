...
### `Alliance Battle` 열은 지금 데이터 소스가 없다

- 스키마: `contribution_type in ('daily_donation','alliance_battle')` — 있음
- 트리거: `alliance_battle_score`를 채우는 코드 — 있음
- 대시보드: 열 — 있음
- **파서: 없음.** `alliance_battle` 행을 만드는 코드가 저장소에 존재하지 않는다

`docs/runbooks/capture-sweep.md`가 `al.battle.rank.info`를 "승격"으로 적어뒀지만
판정만 내리고 파서를 만들지 않았다. 두 문서 모두 2026-08-01에 정정했다.

즉 **실 데이터를 붙이면 그 열은 전부 `—`로 나온다.** 버그가 아니라 미구현이다.
