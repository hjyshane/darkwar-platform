...
**각 번호만 아직 미확인이다.** 5레벨 리듬이 그대로 이어진다고 가정했고 레벨
분포도 그 모양이지만, 각이 붙은 무기를 화면에서 읽어본 적은 없다. 레벨 30짜리가
게임에서 각 1로 안 보이면 성급 쪽은 맞고 각 쪽이 틀린 것이다.
...
**화면으로 대조하지 않았다.** 특히 레벨 100인데 promote가 11 미만인 장비가 있고
지금은 그것을 각 0으로 읽는데, 이 부분이 가장 틀리기 쉽다 — promote 11 미만이
다른 뜻이고 그 위의 수열만 각 트랙일 수도 있다.
