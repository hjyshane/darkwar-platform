...
## 2026-09-25 상태 (15) — (14) 이후 다섯 PR, 그리고 CHECK 제약이 늦게 내리는 핀을 거부했다

(14)를 쓴 직후 `main`에 **#272, #273, #274, #275, #276**이 더 들어갔다. 스키마는
**0176, 0177** 둘이고 나머지는 프런트엔드다. 하이브 브랜치
`claude/hive-formation-tile-assignment-qpopfc`는 #270으로 머지되고 원격에서 지워졌다.
(15)의 작업은 전부 그 뒤 딴 브랜치에서 들어왔다.

### 운영 DB 상태 — 여기부터 확인

| 마이그레이션 | 운영 반영 | 안 올라가 있으면 |
|---|---|---|
| ~0174 | 됨 (2026-09-25) | — |
| **0175** (claim) | (14) 기준 **안 됨** | 캐릭터 연결(`claim_player`)이 운영에서 실패 |
| **0176** (수기 입력) | **확인 안 됨** | Alliance 설정의 `Scores by hand` / `Roster by hand` 두 화면이 죽는다 |
| **0177** (핀 해제) | **확인 안 됨** | **핀 걸린 타일을 비우는 저장이 전부 실패한다** — 아래 |

셋 다 이 세션에서 운영 DB를 들여다볼 방법이 없어 **파일이 main에 있다는 것까지만**
확인했다. 운영 `supabase_migrations.schema_migrations`에서 `20260728000175`,
`...176`, `...177` 세 줄을 직접 보고 빠진 것만 올린다. 올리는 방법은 (14)와 같다 —
**파일 전체 + 기록 행 `insert`를 `begin`/`commit` 하나로**.

**0177이 가장 급하다.** 없으면 아래의 버그가 운영에서 그대로 살아 있다.

순서는 (14)의 규칙이 이번엔 걸리지 않는다: 0175·0176·0177은 컬럼을 더하지 않고
함수와 뷰만 만든다(0176은 `capabilities` / `role_permissions`에 `data.enter` 행도
넣는다). 다만 **0176의 세 함수는 `create function`이라 재실행이 깨진다** — 기록 행을
빼먹으면 나중에 `db push`가 여기서 멈춘다.

### 0174의 실제 버그 — CHECK는 미루지 못한다 (#275, 0177)

0174가 넣은 `hive_pins_protect_somebody`(핀은 지킬 사람이 있어야 한다)는 규칙으로는
맞다. 문제는 **CHECK 제약이 deferrable이 아니라 행이 쓰이는 즉시 검사된다**는 것이다.
두 writer 모두 타일을 비우는 문과 핀을 내리는 문이 **따로**였고, 그래서 비우는 문
자체가 거부됐다:
...
터지는 경우: 핀 걸린 멤버를 옮길 때, 교체할 때, 그리고 #273 이후로는 **핀 걸린 멤버가
연맹을 떠난 뒤 아무 저장이나 할 때마다**. 0177은 규칙을 그대로 두고
`assign_hive_formation_slots`와 `save_hive_formation_layout` 둘 다 **핀을 멤버와 같은
문에서** 내리게 고쳤다. 0173/0174와 나머지는 동일하다.

일반화해 둘 것: **한 문에서 만족시킬 수 없는 불변식을 CHECK로 쓰지 않는다.** 나중에
고치는 문이 있어도 그 문까지 가지 못한다.

### CI db 잡이 초록으로 돌아왔다

| 커밋 | db 잡 |
|---|---|
| `94bee7d` (#270) | 실패 — `supabase start`가 **두 번째 0165**에서 죽음 |
| `076be60` (#271) | 실패 (같은 원인, 고침이 이 커밋에 들어옴) |
| `08795aa` (#275) | **전부 통과** — start / reset / `supabase test db` / 타입 대조 |
| `2f3759e` (#276, 현재 main) | 스킵 (SQL 변경 없음) |

#270 런의 빨강은 **하이브 SQL 탓이 아니었다** — (14)의 0165 중복이다. db 잡이 실제로
끝까지 도는 가장 최근 런은 #275이고 초록이다. `guard`의 버전 중복 거부 단계도 그
뒤 런에서 정상 동작을 확인했다.

### PR별로

**#272 — 옮긴 베이스가 건네받은 멤버와 핀을 지킨다.** draft 타일이 누가 서 있는지를
**저장된 board의 복사본**으로 들고 있었는데, Fill과 드롭다운은 `assignments`에 쓴다.
그래서 마지막 저장 이후 배치한 멤버는 draft 타일에 없었다 — 베이스를 끌면 새 자리에서
캡션이 번호로 되돌아가고, 저장하면 옛 점유자가 다시 쓰이거나 아무도 안 쓰였다. 핀도
옛 오프셋으로 조회돼 같은 식으로 떨어졌다. 이제 draft 타일이 **출처 slot id**를 들고,
누가 어디 있는지는 `assignments`에서 그 id로 읽는다 — 사실의 사본이 하나다.

**#273 — 연맹을 떠난 멤버는 타일을 내놓는다.** 로스터에서 사라져도 타일은 그를 붙들고
있었고, 편집기는 이름을 못 찾아 `?`로 캡션을 달았으며 Fill은 **차 있어 보여서** 손대지
않았다. `vacateDeparted`가 board를 읽는 단계에서 비운다 — 플레이어도 핀도 없애고,
이름은 "누가 떠났는지" 말하기 위해서만 남긴다. 행 자체는 다음 저장 때 정리된다.
**멤버십은 `alliance_roster_latest`로 본다**(CLAUDE.md).

**#274 — 수집기가 못 돌 때 주간 점수와 로스터를 손으로 넣는다 (0176).** officer용 RPC
둘이 새 `data.enter` capability 아래 붙고, **수집기가 쓰는 바로 그 스냅샷 테이블에**
평범한 행을 쓴다(나중에 합치는 단계가 없다). `enter_weekly_scores`는 그 주 리셋
**1초 뒤**로 찍어서, 늦게 동기화된 낮은 캡처보다 항상 그 주 안의 최신이 이긴다;
정정은 두 번째 입력이 더 새로워서 먹는다; 일별 차트는 수기 행을 뺀다.
`set_roster_membership`은 현재 로스터를 한 명 더하거나 뺀 **새 배치로 앞으로** 쓰고
presence를 redacted로 표시한다 — 다음 `al.rank` 캡처가 더 새로워서 대체한다.
pgTAP `92_entered_by_hand_test` 신설.

같은 PR의 테스트 수정 둘이 이 저장소의 함정을 다시 보여준다:
- `92`가 `is_own`을 지우고 제 연맹을 넣었는데, **핀이 없으면** `resolve_own_alliance`
  (0032)가 배치가 들어오는 순간 플래그를 다시 유도해 둘이 되고 `set_roster_membership`이
  (맞게) 거부했다. 이제 admin이 하듯 핀을 박는다.
- `91_claim_player`는 `plan(9)`인데 10개를 돌렸다. **#255와 함께 들어와 한 번도 안
  돌았다** — db 잡이 중복 0165에서 멈춰 여기까지 오지 못했으니까. (14)의 "빨강이
  정보를 잃으면 실패가 쌓인다"가 그대로 반복된 사례다.
- `92`의 power 단언은 함수가 아니라 **순서**가 틀렸다. 로스터 요약이 배치의 power를
  players에 얹으므로, 배치 **앞에** 놓은 값은 덮인다.

**#275 — 핀 걸린 타일을 비울 수 있다 (0177).** 위 참조.

**#276 — 목록에서 멤버를 지도로 끌어다 놓는다.** 한 명 배치하려면 그 타일의 행을 찾아
이름 여든 개짜리 드롭다운을 굴려야 했다. 이제 지도 위에 멤버 목록이 있고(미배치가
먼저), 이름을 끌어다:
- 멤버 베이스 위 — 발자국 **어디든** — 에 놓으면 그 베이스에 앉는다
- 빈 땅에 놓으면 클릭과 똑같이 거기 3x3을 새로 그린다
- 구조물·겹치는 땅·지도 밖은 **이유와 함께 거부**하고, 놓기 전에 포인터 밑 고스트가
  빨개진다

드롭으로 그린 베이스는 저장 전엔 slot id가 없어 `draft:` 대용 키를 받고, 저장이 옮긴
타일에 하던 대로 **위치로** 진짜 id에 맞춘다. 드롭다운은 남는다 — HTML drag and drop은
키보드와 폰에 닿지 않는다.

### 검증된 것 / 안 된 것

- 현재 `main`(`2f3759e`)에서 `pnpm check`(기존 경고 1) / `typecheck` / `test`
  **831 통과** / `build` 초록.
- `scripts/pgtap/run.sh`: **마이그레이션 176개** 적용, 실패 파일 11개는
  `02, 12, 15, 16, 30, 35, 49, 67-70` — 스크립트 헤더가 적어둔 하네스 격차와
  **정확히 같다**. 하이브·수기입력·claim은 전부 통과
  (`91_hive_formation` **70 단언**, `92_entered_by_hand` **37**, `91_claim_player` 10).
- **운영 DB는 이 세션에서 못 봤다.** 위 표의 0175/0176/0177은 사람이 확인해야 한다.
- **#272~#276의 브라우저 확인 여부는 기록에 없다.** (14)가 남긴 숙제 — 운영에서
  **핀 꽂고 → Save → 새로고침 → 핀이 남는지** — 도 확인됐다는 기록이 없으니 그대로다.
- **여전히 합의해서 안 한 것:** 링별 배정 규칙(1링 수동, 2링 계급순, 3링 전투력순…).
  (14)와 같다 — 고친 버전을 써보고 그래도 필요하면 그때.

---

## 2026-09-25 상태 (14) — 하이브 편집기 여섯 묶음, 그리고 번호가 겹친 마이그레이션

(11) 이후 하이브 작업은 같은 브랜치 `claude/hive-formation-tile-assignment-qpopfc`에서
PR 여섯 개로 들어갔다: **#263, #266, #267, #268, #269, #270.** 스키마는 **0173, 0174**
둘뿐이고 나머지는 프런트엔드다. 전부 실제로 써본 사람의 불만에서 나왔다.
...
|---|---|---|
| ~0172 | 됨 | `db push` |
| 0173, 0174 | **됨 (2026-09-25)** | SQL Editor. `schema_migrations`에 기록 행도 손으로 넣었다 |
| **0175** (claim) | **안 됨** | 아래 「번호가 겹쳤다」 |

SQL Editor로 올릴 때는 **파일 전체 + `insert into supabase_migrations.schema_migrations
(version, name) values (...)`를 `begin`/`commit` 하나로** 감싼다. 기록 행을 빼먹으면
나중에 `db push`가 같은 파일을 다시 돌리다 `add column`에서 멈춘다.

**순서 사고 하나:** #270을 머지한 **뒤에** 0174를 올려서, 그 몇 분 동안 운영 Hive
탭이 `pinned` 컬럼을 못 찾아 로드 오류를 냈다. 읽기 실패라 데이터 피해는 없다.
**컬럼을 추가하는 마이그레이션은 머지 전에 올린다** — 새 컬럼은 기본값이 있어서
옛 코드가 신경 쓰지 않지만, 새 코드는 없는 컬럼을 못 읽는다. main 머지가 곧
배포라는 걸(CLAUDE.md) 이 순서에도 적용해야 한다.

### 번호가 겹쳤다 — 0165가 둘 (#255)

`#255`(claim 셀프서비스)가 하이브보다 먼저 딴 브랜치에서 `0165`로 쓰였고, 9/25에
하이브의 `0165_hive_formation_tiles.sql` 위로 그대로 머지됐다. 결과:

- **CI db 잡이 main에서 빨갛다.** `supabase start`가 두 번째 0165에서
  `schema_migrations` 기본키에 걸려 죽는다(`94bee7d`의 run). 같은 로그의
  `toomanyrequests`는 이미지 풀 재시도라 원인이 아니다.
- **운영에는 claim 함수가 없다.** 운영의 `schema_migrations`에 0165는 이미
  (하이브로) 있으니 `db push`는 이 파일을 **적용된 것으로 보고 영원히 건너뛴다.**
  그런데 대시보드는 이미 `rpc('claim_player')`를 부른다 → **캐릭터 연결이
  운영에서 실패한다.**

고친 것: 파일을 **0175로 개명**(내용은 함수 하나라 순서 의존 없음), 참조 주석
셋 정리, 그리고 `guard` 잡에 **버전 중복 거부** 단계. `guard`는 path 필터 없이
매번 돈다. 옛 트리에서 `20260728000165`를 잡고 새 트리에서 비는 걸 확인했다.

**남은 일: 0175를 운영에 올린다** (SQL Editor, 위와 같은 방식). 그리고 #255가 적어둔
대로 `database.types.ts`의 `claim_player`는 **손으로 넣은 것**이라, Windows에서
스택을 켤 수 있을 때 `pnpm db:types`로 모양을 확인할 것.

### 하이브: PR별로

**#263 — 편집기에서도 빈 땅 드래그가 지도를 민다.** `Move a base` 모드에서
빈 땅 드래그는 원래 아무 뜻이 없었다. 베이스 위면 든다, 밖이면 민다 — 조건이
겹치지 않아 추측이 필요 없다. 함정: "이동은 뒤따르는 클릭을 삼킨다" 규칙이 그대로면
빈 땅 클릭(타일 놓기)이 죽는다. 그래서 pan이 `viaModifier`를 들고, 안 움직인
평범한 press는 여전히 클릭이다.

**#266 — 500타일 벽, 좌표축, 3x3만 배정.** 500은 데이터가 아니라 `fetchBoard`의
`.limit(500)`이었다. 이제 1,000행씩 **페이지로 읽고**(`everyPage`, ORDER BY에 고유
tiebreak), X/Y 라벨이 그리드 밖에 붙는다. `isMemberBase` = kind `base` **그리고 3x3** —
표·자동 채우기·지도 캡션·저장 순서가 전부 이걸 쓴다.

**#267 — 축 가독성, 정직한 `#`, 커질 수 있는 포메이션 (0173).**
- 라벨 수를 고정 10개가 아니라 **축 폭에서** 계산(가로 30px, 세로 22px당 하나).
  프레임 폭으로 재면 그리드는 720px에서 멈추는데 프레임은 전폭이라 23개가 들어간다.
- 표의 `#`가 `slot.ordinal`(구조물 포함)이라 Frankie가 1번을 먹었다. 이제 표 아래로
  세는 번호 = 지도 캡션 번호.
- 한계는 **둘**이었고 둘 다 저장 공간이 아니었다. 그리기: 화면 밖 타일을 **그림에서만**
  컬링(번호·링·표는 전체 기준). 저장: 0167의 겹침 사전검사가 전 쌍 자기조인이라
  20,000타일에서 39초, 그게 지키는 insert는 1.2초. **친절한 오류 문구가 일의 20~30배**였다.
  0173은 1,000타일 이하에서만 사전검사를 하고 그 위는 배제 제약이 혼자 거부한다.
  허용되는 것은 안 바뀐다. 상한 5,000 → 20,000.
- 잘못된 이유로 통과한 테스트: 처음엔 **같은 오프셋 반복**으로 겹침을 만들었는데
  함수가 오프셋으로 매칭하니 한 행으로 합쳐져 겹침이 없었다. 겹침은 **다른 오프셋의
  발자국이 만나야** 한다.

**#268 — 링은 다시 가운데서 잰다, Save 하나, 표가 누군지 말한다.**
진짜 버그 하나가 "레이어" 불만 대부분을 설명한다: `ringOf`가 **가장 가까운 구조물**까지
거리였는데, #260 이후 경계선·`keep clear`도 구조물이라 가장자리 베이스가 링 1이 됐다.
이제 **앵커 위의 것(Frankie)**, 없으면 앵커 타일에서 잰다. 옛 코드에서 실패하는 테스트 둘.
Save 버튼은 하나로 합쳤다(`Saved` / `Save who goes where` / `Save the shape` /
`Save the shape and the people`) — 호출은 여전히 둘이다, 타일은 쓰기 전엔 id가 없으니까.
`Ring` 열, 미배치 멤버 **이름** 목록, 드롭다운에 `R4 · 48.0M`.
**합의해서 안 한 것:** 링별 배정 규칙(1링 수동, 2링 계급순, 3링 전투력순…). 고친 버전을
먼저 써보고 여전히 필요하면 그때.

**#269 — 저장해도 핀이 남고, 그룹으로 꽂는다.** 저장 후 재조회가 핀 맵을 통째로
지웠다. 이제 **타일이 사라졌거나 다른 사람이 서 있을 때만** 떨어진다(`survivingPins`,
옛 동작에서 빨개지는 테스트 둘). `Pin every filled tile` / `Pin R4 and R5` /
`Unpin everything`, 전부 **더하기**이지 교체가 아니다.

**#270 — 핀이 행에 산다 (0174), 목록은 이름순.** 핀은 브라우저 상태라 새로고침에
죽고 다른 officer에게 안 보였다. `hive_formation_slots.pinned boolean` — `(타일, 멤버)`
쌍이 아니다, 멤버는 이미 행에 있다. **빈 타일의 핀은 체크 제약과 RPC 둘 다 거부**
(다음 자동 배정 대상에게 함정이 된다). 뷰 컬럼은 **끝에** 붙인다 — 중간 삽입은
`create or replace view`가 rename으로 읽고 거부한다(0170에 이어 두 번째).
클라이언트 쪽 둘: 핀은 행에서 시드하되 브라우저 것과 **병합**, 레이아웃 저장은 핀을
**id가 아니라 오프셋**으로 들고 간다(옮긴 타일은 delete+insert라 id가 바뀐다).
헤더 전체선택 체크박스(부분이면 indeterminate). 목록 기본은 이름순, `#`는 **정렬 전에**
매긴 채움 번호라 두 보기 모두 지도 캡션과 같다.

### 검증된 것 / 안 된 것

- 각 PR에서 `pnpm check`/`typecheck`/`test`(최종 793)/`build`, pgTAP은
  `scripts/pgtap/run.sh`로 `91_hive_formation_test` **67 단언**.
- **저장 경로는 브라우저로 한 번도 못 돌렸다.** 픽스처 빌드에선 저장이 네트워크로
  나가 실패한다. `survivingPins`를 순수 함수로 뺀 이유가 이거다. 운영에서 **핀 꽂고 →
  Save → 새로고침 → 핀이 남는지**를 사람이 한 번 봐야 한다.
- #266의 축 정렬·500타일 초과는 PR 시점에 브라우저 미확인이었고, #267에서 축은 봤다.

---

## 2026-09-11 상태 (13) — 데스크톱 앱이 main에 들어왔다 (PR #258, 커밋 63개)

**다른 플레이어에게 건넬 수 있는 Windows 앱이 생겼다.** `6122be9`로 머지했고,
`apps/desktop/`이 그 결과다. 브랜치 `file-upload-location-sharing-5fca93`에서
63커밋이 한 번에 들어왔으니, 이 문서에서 이 항목이 그 브랜치의 유일한 요약이다.

### 무엇인가

Tauri v2 창 하나 + PyInstaller로 묶은 사이드카(`dw-sidecar.exe`) 하나.
창은 사이드카의 loopback HTTP만 말하고, 사이드카는 **그 플레이어 자신의 로컬
저널(SQLite)** 만 읽는다. Supabase도 클라우드도 타지 않는다 — 설계 자체가
그렇고, 근거는 `docs/adr/0001-user-app-is-local-only.md`에 있다.

화면 여섯: 검색 / 지도 / 플레이어 프로필 / 연맹 로스터 / 아레나 대진 / 설정.
설정에서 dumpcap을 **앱의 자식 프로세스로** 띄우고 끄며(예약 작업이 아니다),
창을 닫으면 프로세스 트리째 정리한다. 지도는 대시보드와 **같은 투영 코드**를
쓴다 — `packages/ui`로 뽑아냈고, 두 렌더러가 같은 좌표를 낸다는 테스트가 있다.

개발 중 실행은 `apps/desktop/`에서 `pnpm app`(= `tauri dev`).

### 배포 상태 — 0.1.0, 서명 없음

아이콘·CSP·NSIS(사용자 단위 설치, 권한 상승 없음)·첫 실행 라우팅(빈 검색창이
아니라 설정으로)까지 끝냈고, **설치 → 실행 → 제거를 실제로 한 번 돌렸다.**
제거해도 앱데이터와 저널은 남는 것까지 확인했다.

서명은 **의도적으로 안 했다**(인증서 연 $200~400, 8명 쓰는 사적 도구). 대가는
문서로 갚는다 — SmartScreen 경고가 뜨고, PyInstaller + 패킷 캡처 조합이라
백신 오탐 가능성이 낮지 않고, 자동 업데이트가 없어 새 버전은 손으로 받는다.
**미리 말해주지 않으면 악성코드로 알고 지운다.** 무슨 화면이 뜨는지, Npcap을
먼저 깔아야 한다는 것까지 `docs/runbooks/releasing-the-desktop-app.md`에 있다.

**검증 안 된 것: 두 번째 머신.** 설치를 성공시킨 건 개발 머신 한 대뿐이다.

### 사이드카 바이너리는 저장소에 없다 (`.gitignore`)

릴리스마다 `uv run pyinstaller services/collector/dw-sidecar.spec`로 다시 만들어
`apps/desktop/src-tauri/binaries/dw-sidecar-x86_64-pc-windows-msvc.exe`로
복사해야 한다. **이 저장소를 네 번 문 실수가 바로 이걸 건너뛴 것이다** —
빌드는 성공하고, 창도 뜨고, 방금 고친 기능만 조용히 옛날 코드로 돈다.
이 워크트리에 있던 2026-09-07자 빌드(와 5.0 GB짜리 `target/`)는 지웠으니,
다음 빌드는 어차피 새로 만들게 된다.

### 머지하면서 드러난 것 둘 — 앞으로도 물릴 자리

**1. `sys.platform` 가드 + mypy `warn_unreachable`은 한쪽만 통과시킨다.**
`if sys.platform != "win32": ...; return` 뒤의 코드는 Linux CI에서 "도달 불가"로
빨개진다. 뒤집으면 이번엔 Windows에서 반대쪽이 빨개진다. 개발은 Windows,
CI는 Linux라 **양쪽 다 검사받으므로 guard clause로는 못 맞춘다.** `if/else`여야
한다 — mypy는 플랫폼이 안 맞는 가지를 조용히 버린다. `598117f`.

**2. 잡이 앞 단계에서 죽으면 뒷 단계는 "통과"가 아니라 "미실행"이다.**
python 잡이 mypy에서 멈춰 있던 동안 **pytest는 Linux에서 한 번도 안 돌았다.**
mypy를 고치자마자 경로 구분자 테스트가 터졌다 — `Path(r"C:\DW_data\live")`는
Linux에서 PosixPath라 `/`로 이어붙고, 테스트가 `\`를 박아뒀다. 코드가 아니라
기대값을 코드와 같은 방식으로 만들게 고쳤다(`bb744ca`). 빨간 잡을 볼 때
**어디서 멈췄는지**를 먼저 봐야 한다.

덤으로, main의 하이브 작업(#259~#263)과 붙은 충돌 네 건은 전부 같은 모양이라
**main 파일을 통째로 받고 import 한 줄만 `@dw/ui`로 다시 가리켰다.** 브랜치가
그 파일들에 기여한 건 그 한 줄뿐이었다.

### 남은 것

- 두 번째 머신에서 설치 검증 (누구에게 건네기 전에)
- 서명·자동 업데이트 — 보류. 사면 무엇이 바뀌는지는 런북에 적어뒀다
- 사이드카는 **읽기 경로만** 묶는다(spec의 `excludes`). 앱에 캡처를 넣게 되면
  거기서 다시 판단할 것 — 크기가 두 배가 되고 Npcap이 딸려 들어온다

관련 런북: `releasing-the-desktop-app.md`, `desktop-sidecar-lifecycle.md`,
`local-screens.md`, `map-agreement.md`.

---

## 2026-09-09 상태 (12) — 타입 생성을 `--local`로 통일했다

(9)부터 "남은 결정"으로 끌고 오던 것. 결론부터: **`--local`이 유일한 길이고,
`--linked`는 이제 도구가 거부한다.**

### 먼저, 내가 (10)에 적어둔 진단이 틀렸다

(10)의 남은 것에 "저장소 스크립트(`pnpm db:types`)가 `--linked`"라고 썼는데
**거짓이다.** `package.json`을 열어보지 않고 증상에서 추론해 적었고, 사용자에게
두 번 그렇게 말했다. `git log -L`로 확인하면 이 스크립트는 도입된 날부터
`--local`이었고 CI도 `--local`이다. 둘은 한 번도 갈린 적이 없다.

**진짜로 갈렸던 건 CI와 *운영자가 실제로 치던 명령*이다.** (7)에 그대로 적혀
있다 — Windows에 로컬 스택이 없어서
`supabase gen types typescript --linked --workdir C:\darkwar-platform`로
재생성했다고. 호스티드 프로젝트는 `__InternalSupabase` / `PostgrestVersion`
블록을 내고 로컬 스택은 안 내니까, 그렇게 만든 파일은 CI의 `--local` diff 검사에
아무도 손대지 않은 블록으로 걸린다.

증상만 보고 원인을 지목하면 이렇게 된다. **`package.json` 한 줄 여는 게
추론보다 쌌다.**

### 고친 것 둘

**1. `pnpm db:types`가 `scripts/db-types.mjs`를 거친다.**
`>` 리다이렉트는 명령이 실행되기 *전에* 파일을 비운다. Docker Desktop이 꺼져
있으면 CLI가 실패하는데 커밋된 타입 파일은 이미 비워진 뒤고, 운영자는 망가진
작업트리와 "아무것도 안 한" 명령을 앞에 두고 `--linked`로 손을 뻗게 된다. 그게
이 사고의 실제 경로다.

새 스크립트는 stdout을 받아 **검사한 뒤에만** 쓴다. 실패하면 아무것도 안 쓰고,
`--linked` 모양(`__InternalSupabase:` / `PostgrestVersion`)이면 그것도 거부한다.
실패 메시지가 `supabase start`를 시키고 `--linked`를 쓰지 말라고 명시한다.

**2. `pnpm test`가 커밋된 타입의 모양을 본다.**
`apps/dashboard/tests/generatedTypes.test.ts`. 손으로 고쳤든 머지로 들어왔든
CLI를 직접 쳤든, `--linked` 산출물이면 **커밋 전 루프에서** 걸린다 — `db` 잡
6분을 기다리지 않고. `web` 잡의 path 필터가 `packages/**`를 포함하니 CI에서도
탄다.

주의: `Omit<Database, "__InternalSupabase">`는 `--local`도 내는 정상 보일러플레이트다.
그래서 이름이 아니라 **선언**(`__InternalSupabase:`)을 본다. 이름으로 매칭하면
멀쩡한 파일이 빨개진다.

둘 다 무의미한 단언이 아닌지 확인했다 — 가짜 `__InternalSupabase` 블록을 넣어보고
테스트가 실제로 실패하는 것, 그리고 스택 없이 스크립트를 돌렸을 때 타입 파일이
`git diff` 기준 그대로인 것을 봤다.

### 그래서 Windows에서 할 일

타입을 재생성할 일이 생기면 **Docker Desktop을 켜고** `supabase start` 뒤에
`pnpm db:types`. 스택이 없으면 재생성하지 말고 그냥 두면 된다 — 스크립트가
파일을 망가뜨리지 않는다.

---

## 2026-09-09 상태 (11) — 편집 도구, 지도 이동, 그리고 지형은 없다 (PR #260)

(10)을 머지한 뒤 같은 브랜치를 main에서 다시 따서 올린 두 번째 묶음.
**스키마 변경 없음** — 전부 프런트엔드이고, `fb92ea5`로 머지했다. `db push`도
필요 없다.

### 드래그가 뜻하는 것이 늘었다

`What a drag does`에 모드 다섯 개: **Move a base / Fill an area / Draw a
boundary / Erase an area / Select**.

**한 제스처는 두 뜻을 가질 수 없다**는 게 설계의 전부다. 베이스 위의 press는
"이걸 든다"와 "이 영역을 쓸어담는다"가 눌린 순간 구분이 안 되므로, 추측 대신
모드로 갈랐다. 영역 도구가 켜져 있으면 베이스 드래그는 꺼진다.

- **채우기는 이미 그려진 것을 피해서** 들어간다. 사각형 하나로 놓으면 베이스
  하나만 걸쳐도 통째로 거절되는데, 비워둘 구역을 긋는 이유가 대개 바로 옆
  하이브다.
- **경계선은 테두리만** 찍는다. 40x40이면 채우기 1,600칸 대 테두리 156칸이고,
  채우기는 포메이션 타일 예산을 통째로 먹는다.
- **선택은 박스에 완전히 들어간 것만** 잡는다. 걸친 것까지 잡으면 한 칸 넘긴
  드래그에 본 적 없는 타일이 딸려온다.

**그룹 이동에서 걸린 것 셋** — 다음에 비슷한 걸 만들면 또 밟는다:
1. 그룹은 **자기 옛 위치와 충돌하면 안 된다.** 20칸을 한 칸 밀면 새 자리가 전부
   자기가 비운 옛 자리와 겹친다. 움직이는 집합은 *가만히 있는 것*하고만 비교한다.
2. 드래프트가 오프셋을 키로 쓰므로 **제자리 수정이 불가능하다.** 전부 빼내고 전부
   다시 넣는다 — 0167이 DB에 대해 한 얘기를 `Map`에서 반복한 것.
3. **선택이 타일을 따라 새 키로 옮겨가야** 연속으로 밀 수 있다.

### 격자 문제는 이번엔 측정해서 고쳤다

두 번 틀렸던 것이다. `.tile-grid`는 `<button>`이고 전역 `button:hover`가
`background` **단축 속성**을 쓴다. 단축 속성은 포함하는 롱핸드를 **전부** 리셋하는데,
1차 수정이 색과 이미지만 복구하고 `background-size`를 빠뜨려서 그라디언트가
`auto`로 떨어졌다 — 왼쪽에 선 하나, 위에 선 하나. 사라진 게 아니라 한 칸짜리가
된 것이라 "고쳤다"고 보고까지 했다.

**교훈은 다시 서술하지 말고 구조를 바꾸라는 것이다.** 지금은 선언 블록 하나에
평상시 셀렉터와 hover 셀렉터를 같이 적어서 일부만 복구하는 코드를 못 쓰게 했고,
`index.css`를 읽어 검사하는 테스트(`tests/gridPaint.test.ts`)를 붙였다.
`background-size`를 지워보고 실제로 빨개지는 것까지 확인했다(무의미한 단언 방지).

**ctrl+드래그(또는 가운데 버튼)로 지도 이동.** 모드가 아니라 수식키다 — 이동은
다른 걸 하다가 중간에 하는 동작이라 모드면 쓰던 도구를 나갔다 와야 한다. 멤버용
읽기 전용 지도는 그냥 드래그로 움직인다.

### 지형 조사: 연맹 센터는 있고, 도시 영역 레벨은 없다

하이브 뒤에 실제 지형을 깔 수 있는지 물어서, 커밋된 `world.get.new` 픽스처
2개(922타일)를 저장소 파서로 디코드해 확인했다. **전문은
`docs/runbooks/season-map-capture.md`에 있다.** 요약:

- **연맹 센터는 이미 디코드되고 그냥 버려진다.** 타입 15가 연맹 태그·이름과
  세 센터를 구분하는 `buildId`(41000/42000/43000)를 들고 있다. `Tile.raw`까지는
  오는데 `normalize/world_map.py`가 타입 3·6이 아니면 `continue`로 버린다.
  **프로토콜 작업은 남은 게 없다** — 테이블과 writer만 만들면 된다.
- **도시 영역 레벨(1~6)은 어느 페이로드에도 없다.** 모든 오브젝트 타입에서 값이
  1..6에만 있는 필드를 찾았더니 922타일 중 딱 하나 나왔고 그건 영역이 아니라
  도시 서브메시지의 `f3.23.2`(값 두 개)였다. **영역은 지형이라 변하지 않으니
  서버가 뷰포트마다 다시 보낼 이유가 없다 — 스윕을 아무리 돌려도 안 나온다.**
- 그래서 **둘 다 만들지 않기로 했다.** 경계는 위의 `Draw a boundary`로 손으로
  긋는다. 이게 사용자의 결정이다; 다시 조사하지 말 것.
- 덤: 그 런북의 타입 표가 타입 6을 "marches"라고 적어둔 건 낡았다.
  `protocol/worldmap.py`가 근거를 들어 "멤버 시즌 건물"로 정정했고, 이번 스캔도
  파서 쪽이 맞다고 나왔다(uid와 목적지 point id를 든 건 타입 **7**).

---

## 2026-09-09 상태 (10) — 하이브 편집기: 타일이 제 크기를 갖고, 모양이 목록이 됐다

(8)에서 만든 하이브 포메이션 위에 세 덩어리를 얹었다. 마이그레이션 **0169~0172**.
**PR #259로 머지했고(`78b59b9`), `db push`도 끝났다** — `migration list --linked`에
0169~0172가 전부 Remote로 찍힌 것을 확인했다. 이 절에 할 일은 남아 있지 않다.

### A — 타일이 제 크기·종류·색을 갖는다 (0169, 0170)

0165는 전부를 숫자 하나 위에 지었다. 베이스는 3x3이니 배제 제약이
`int4range(dx - 1, dx + 2)`였고, 프랭키(4x3)를 피하려고 프런트엔드가
`CENTRE_BOX`를 하드코딩했다. **정작 중요한 규칙 — 아무것도 겹치지 않는다 — 이 두
곳에, 두 언어로, 그중 하나만 데이터베이스에** 있었던 것이다.

0169가 크기를 행으로 옮기고 제약이 그걸 읽는다. 프랭키는 특수 케이스가 아니라
`span 4x3, kind='structure'`인 평범한 타일이 되고, 연맹 건물과 1x1 마커는 같은
것의 다른 크기가 된다.

**짝수 span의 좌표 규약은 적어둔다.** `low = dx - (span - 1) / 2` (정수 나눗셈).
3이면 가운데, 4면 **가운데 두 열 중 서쪽**, 1이면 그 타일 자신. SQL과 TypeScript가
같은 식을 쓴다. 프랭키의 어느 열이 게임 좌표인지는 확인 못 했으므로 서쪽으로
가정했고, 틀렸다면 화면에서 보이도록 사각형을 그린다.

### B — 지도 위의 것들이 목록이 된다 (0171)

`hive_map_features`: **이름과 모양뿐**이다. 위치도 서버도 없다.

- **위치 없음.** 목록은 "연맹 HQ는 3x3"이라고만 말한다. *우리* HQ가 어디 서 있는지는
  한 서버의 한 포메이션 이야기이고 `hive_formation_slots`에 있다.
- **서버 없음.** 같은 건물은 여덟 지도에서 같은 크기다.
- **놓인 타일이 목록을 참조하지 않는다.** 이름은 배치 순간 슬롯의 label로
  **복사**된다. 다음 주에 'Depot'을 고쳐 부른다고 이미 여든 명에게 보낸 좌표의
  캡션이 바뀌면 안 된다. 없는 외래키를 pgTAP이 단언한다 — "안 만들기로 했다"와
  "빠뜨렸다"는 스키마에서 똑같이 보이니까.

색 목록을 도메인으로 공유하지 않고 두 테이블에 각각 썼다(도메인 타입 컬럼은
PostgREST 타입 생성이 달라진다). **두 제약의 정의문이 문자열로 같은지를 단언**하는
것이 두 목록을 하나로 유지한다.

### C — 모양이 포메이션보다 오래 산다 (0172)

`hive_formation_templates`: 앵커도 서버도 없고 **아무도 서 있지 않은** 오프셋
목록. 0165의 "슬롯은 좌표가 아니라 오프셋"을 끝까지 밀면 나오는 결론이다.

**적용은 쓰기가 아니라 드래프트 편집이다.** 템플릿에는 앵커가 없으니 타일이 지도
안에 들어가는지는 *놓이는 포메이션만* 답할 수 있다. 그래서 불러오기는 화면의
드래프트를 바꾸고, 몇 개가 가장자리 밖인지 세어 말해주고, 앵커를 옮긴 뒤 평소의
`save_hive_formation_layout`으로 저장한다. 여기서 바로 저장했다면 아직 보이지도
않는 타일 하나를 지목하며 전체를 거절했을 것이다.

**저장된 모양은 사람을 기억하지 않는다.** 2주 전 모양이 누가 어디 섰는지 기억하면
새 땅 위의 낡은 로스터가 된다 — 보드에 `still_a_member` 플래그가 있는 이유가
그거다. 채우는 건 오늘의 로스터로 자동 배정이 한다.

### 배포하면서 알게 된 것

**`db push` 로그의 Docker 에러 벽은 진짜로 무시해도 된다.** Docker Desktop이 꺼져
있으면 `npipe:////./pipe/dockerDesktopLinuxEngine` 실패가 두 줄 뜨는데, push 자체는
원격 Postgres에 직접 붙어서 계속 진행된다. 판단은 그 줄이 아니라 마지막 줄
(`Finished supabase db push.`)로 하고, 확실히 하려면
`supabase migration list --linked`를 보면 된다. 이번에 그렇게 확인했다.

### 남은 것

- ~~**타입 생성이 `--local`과 `--linked`로 갈린다.**~~ **정리했다 — (12) 참조.**
  그리고 이 줄 자체가 틀렸었다: `pnpm db:types`는 처음부터 `--local`이었다.
  갈렸던 건 스크립트와 CI가 아니라 **CI와 운영자가 실제로 치던 명령**이다.
- `scripts/pgtap/run.sh`는 171개 마이그레이션을 적용하고 91번이 59개 단언으로
  통과한다. 실패 11건은 헤더에 적힌 하네스 한계 그대로다.

---

## 2026-09-08 상태 (9) — CI의 db 잡이 처음으로 초록이 됐다 (0168 + 테스트 10개, PR #257)

(8)의 하이브 작업을 머지한 뒤, **Actions가 빌링 실패 없이 실제로 돌기 시작한 걸
발견했다.** CLAUDE.md의 "Actions는 4초 만에 빨개진다, 로컬 게이트가 유일한
게이트"는 이제 낡았다 — 지금은 CI가 진짜로 검증한다.

그리고 돌자마자 `db` 잡이 **열 개 파일**에서 실패했다. 085dcf9(main) 기준으로
파일·테스트 번호까지 동일한 집합이었다. 즉 **내 브랜치 탓이 아니라 main이 몇 주째
그 상태였다.**

### 결론부터: 아홉은 낡은 테스트, 하나는 진짜 발견

| 파일 | 원인 |
|---|---|
| 28 | 0161이 멤버십을 `alliance_roster_latest`로 옮겼는데 로스터 스냅샷이 **0개**. 아무도 채점 안 됨 |
| 40 | 로스터 배치 시각이 엇갈려 최신 배치에 **1명뿐**. + 0164의 `tier_reason` 문구 변경 |
| 42 | 0151이 "떠난 사람" 픽스처를 추가하고 단언은 안 고침. 스칼라 서브쿼리가 2행을 받아 **에러로 파일을 죽임** |
| 43·90 | `scoring_version` 4·6 고정 (현재 8) |
| 48 | cross_server 판독이 3개로 늘어 climb 1→2 |
| 52·54 | 0163이 `rank_period_latest`를 "기간별 최신 버전"으로 재정의. 두 파일 다 폐기된 계약을 단언 중 |
| 88 | `unnest` 배열 6개인데 `plan(14)`, 실제 16 |
| **87** | **0065의 default privileges 갭 — 테스트가 옳았다** |

### 87: 0065가 SELECT만 회수했다

```sql
alter default privileges in schema public revoke select on tables from anon;
```

Supabase 기본값은 anon에게 **네 개**를 준다. 하나를 빼면 **셋이 남는다** —
INSERT/UPDATE/DELETE가 0065 이후 생성된 모든 테이블·뷰에 CREATE 시점에 조용히
상속됐다. **95개 관계**가 갖고 있었다.

**한 달 동안 자기 테스트가 초록인 채로 살아남은 이유가 핵심이다.** 0065의 test
계층인 `34_no_public_read_test`는 *읽을 수 있는가*를 묻는다. 스키마 전체를 훑고
새 테이블도 설계대로 잡아왔지만 **쓰기 권한은 읽기가 아니다.** 이걸 본 유일한
단언이 87이고 그건 뷰 하나만 본다.

**0168**이 권한을 회수하고 기본값을 닫았고, **34를 "anon은 public에서 어떤 권한도
갖지 않는다"로 넓혔다.** 좁은 단언이 진짜 결함이고 권한은 그게 못 본 것이다.

**노출 정도는 과장하지 말 것 — 구멍은 없었다.** 모든 테이블에 RLS가 켜져 있고
anon을 named하는 정책이 없어서 정책 없는 grant는 아무것도 못 쓴다. RLS를 우회하는
길은 `security_invoker`가 아닌 갱신 가능 뷰인데 public에 하나(`notification_channel_names`)
있고 0125가 이름을 지정해 회수해뒀다. **열린 문이 아니라 없는 난간이었다.**

### 반복되는 함정 셋 — 다음에 또 밟지 말 것

**1. 테스트에 scoring_version을 고정하지 마라.** 43·90이 걸렸는데, 40 파일 안에
이미 *"버전 3으로 고정했다가 0075가 4를 쓰는 순간 깨졌다"*는 주석이 있다.
**같은 사고가 세 번째다.** 셋 다 `rank_period_latest`를 읽도록 통일했다.

**2. 부재를 검사하는 단언은 조용히 만족된다.** 90의 assertion 7과 40의 assertion
3은 null을 기대하는데 **행이 사라진 내내 통과했다.** 버전 고정은 단언을 깨뜨리기만
하는 게 아니라, null을 기대하는 단언을 진공으로 통과시킨다. 실패 목록만 보면 안 
보인다.

**3. 로스터 픽스처는 한 배치에 한 captured_at.** `alliance_roster_latest`는 최신
순간을 **공유하는** 행들을 낸다. 두 멤버를 한 순간 어긋나게 쓰면 2인 로스터가 아니라
1인 로스터다. 28·40·91이 전부 이걸로 걸렸다.

### Docker 없이 pgTAP을 돌리는 법 — `scripts/pgtap/run.sh`

이 진단이 가능했던 건 **전체 마이그레이션 체인을 로컬 PostgreSQL 16에 올려서
실제로 돌렸기** 때문이다. 추측이 아니라 재현이었고, 편집 전 실패 번호가 CI와 정확히
일치하는 걸 확인하고 시작했다.

스크립트로 커밋해뒀다. Docker 불필요, PostgreSQL 16 + `postgresql-16-pgtap`만
있으면 된다:

```
./scripts/pgtap/run.sh
```

- `pg_net`·`pg_cron` 두 확장만 스텁한다. 호출부가 전부 plpgsql 본문 안이라
  마이그레이션·테스트 시점에 파싱되지 않는다.
- `scripts/pgtap/supabase-stub.sql`이 0001 이전에 Supabase가 제공하는 것(auth,
  storage, API 롤, **default privileges**)을 흉내 낸다.
- **CI와 불일치하는 11개는 이 하네스 탓이다**(파일 상단에 목록과 이유가 있다).
  여기서 나는 **새로운** 실패는 믿을 것, CI와의 **불일치**는 스텁을 먼저 의심할 것.

pgTAP은 `extensions` 스키마에 들어가므로 `search_path`에 넣어야 `plan()`이 보인다.
안 그러면 31개가 전부 `function plan(integer) does not exist`로 죽는다.

### 처음 실행된 검사 하나 — 그리고 남은 결정

pgTAP을 초록으로 만들자 **그 뒤에 있던 생성 타입 검사가 이 저장소에서 처음
실행됐다.** `db` 잡은 `supabase test db` **다음에** 그걸 하는데, 그 앞이 계속
실패하고 있었으니 한 번도 도달한 적이 없었던 것이다. 도달하자마자 손으로 쓴
`Relationships`가 틀렸다고 정확히 짚었고, CI가 출력한 diff를 그대로 반영했다.

**남은 결정 하나** — 이건 브랜치 문제가 아니다:

- CI는 `supabase gen types typescript --local`을 돌리고, 이건
  `__InternalSupabase` / `PostgrestVersion` 블록을 **안 낸다**.
- 커밋된 파일엔 있었다. (7)에 적힌 대로 로컬 스택이 없어 `--linked`로
  재생성하는데, 호스티드 프로젝트는 **낸다**.

**두 명령이 이 파일에 대해 서로 다른 답을 내고, 검사는 그중 하나만 본다.** 지금은
게이트가 돌리는 쪽에 맞춰 제거했지만 **다음 `--linked` 재생성 때 다시 들어가 또
실패한다.** Docker 생기면 `--local`로 통일하든, 검사가 그 블록을 무시하게 하든
정해야 한다. 다음 마이그레이션 전에.

### 프로덕션에 아직 안 올라간 것

**`supabase db push`로 0165~0168 넷.** (8)의 하이브 셋이 코드만 배포되고
마이그레이션은 안 올라간 상태이고, 여기에 0168이 더해졌다.

- 그때까지 **Hive 탭만** `Could not load formations: ...`로 뜬다. 다른 탭은
  hive 테이블을 안 건드린다.
- push 직후 1분쯤 PostgREST 스키마 캐시 때문에 같은 증상이 남을 수 있다 —
  아래 (기존 항목)에 적힌 알려진 현상이고 `NOTIFY pgrst, 'reload schema';`로 풀린다.
- 0168은 **회수**라서 적용이 뭘 넓힐 일은 없다.

### 실측

| | |
|---|---|
| CI `db` 잡 | **초록** — `Files=91, Tests=1030, Result: PASS` |
| 로컬 하네스 | 마이그레이션 167개 적용, CI 실패 10개 전부 초록 |
| 0168 전후 차이 | 34·87 두 파일뿐, 나머지 전부 불변 |
| 넓힌 34 | 0168 없으면 **실패한다** (항진명제가 아님을 음성 대조로 확인) |

---

## 2026-09-08 상태 (8) — 하이브 포메이션: 타일 배정 (0165~0167 + 프론트)

### 요청 — "맹원들이 자기 멋대로 텔포해서 줄이 안 맞는다"

하이브 이동이나 전쟁 집결 때 베이스가 정렬이 안 되는 건 의지 문제가 아니라
**할 수 있는 방법이 없어서**다. 채팅으로 "여기로 모여"를 말하면 84명이 각자
비어 보이는 타일을 고르고, 게임은 좌표 하나만 보여준다. 그 좌표가 실제로는
**9칸**이라는 사실이 어디에도 안 적혀 있다.

**베이스는 3x3이고 좌표는 그 한가운데다.** 이 한 문장이 이 기능 전체의
근거다. 맵은 한 칸 단위로 주소가 매겨지므로 `500,500`과 `501,500`은 서로 다른
두 칸의 이름이면서 **같은 땅**이다. 둘 다 배정받으면 두 번째 사람은 텔포가
안 되고, 그 사실을 여기가 아니라 게임 안에서 알게 된다.

### 0165 — 겹침을 DB가 거부한다

중심 좌표가 **두 축 모두에서 3칸 미만**이면 겹친다. 한 축만 가까운 건 겹침이
아니라 **꽉 찬 하이브**이고 그게 원하는 모양이다. 이걸 규칙이 아니라
exclusion constraint로 들고 있다:
...
`btree_gist`가 uuid `=` 때문에 필요하고, opclass는 search_path에 기대지 않게
스키마를 붙여서 적었다.

**슬롯은 절대 좌표가 아니라 앵커로부터의 오프셋으로 저장한다.** 하이브는 한 번
그리고 여러 번 옮긴다 — 84칸을 옮기는 게 한 행 수정이고, 모양이 안 변한다는 게
증명된다. 부수 효과로 **겹침 판정이 오프셋만의 성질**이 된다(앵커는 상수라
소거된다). 그래서 위 제약이 dx/dy만 보고도 땅에 대해 참이다.

3x3이 맵 밖으로 나가는 경우는 check constraint로 못 잡는다(슬롯은 부모의
앵커가 필요하고, 앵커는 모든 자식이 필요하다). **트리거 두 개, 함수 하나**로
양쪽에서 본다. 앵커 이동이 84칸을 한꺼번에 맵 밖으로 밀어내는 유일한 연산이라
그쪽이 특히 필요했다.

### 0166 — 좌표 덧셈은 서버에서 한 번만

`hive_formation_board`가 `anchor + offset`을 한다. 브라우저에서 더하면 이
기능이 내놓는 유일한 숫자가 클라이언트 계산 뒤에 숨고, 캐시에 낡은 앵커가
있으면 84명이 엉뚱한 땅으로 간다.

`still_a_member`는 **`alliance_roster_latest`로** 본다. 저장소 상시 규칙 그대로
`players.current_alliance_id`는 아무도 안 지우므로 항상 "예"라고 답한다. 2주 전
포메이션에 탈퇴자가 배정돼 있는 걸 알려주는 게 이 컬럼의 존재 이유라서 거기서
읽으면 의미가 없다. EXISTS라 슬롯당 한 행이 유지된다.

### 0167 — 저장 단위는 "포메이션 전체"

0165의 제약 때문에 **부분 쓰기가 불가능하고, 그게 맞다**:

- 모양을 동쪽으로 한 칸 밀면 새 슬롯이 전부 옛 슬롯과 겹친다. insert→delete는
  첫 행에서 거부되고, delete→insert는 그 사이에 아무것도 안 끼어야 한다.
  PostgREST로는 두 요청 = 두 트랜잭션이다.
- 두 명 자리 바꾸기는 `hive_slot_one_place_per_member`가 **어느 순서로 해도**
  중간 상태에서 거부한다.

그래서 `save_hive_formation_layout` / `assign_hive_formation_slots` 두 함수가
한 트랜잭션에 통째로 쓴다. 살아남은 타일의 배정은 유지되고, 사라진 타일 때문에
자리를 잃은 인원 수를 결과로 돌려준다.

**temp table을 쓰지 않는다.** `set search_path = ''`면 pg_temp가 안 잡히고,
함수 안에서 만든 temp table은 같은 트랜잭션 안 두 번째 호출에서 터진다 —
테스트 스위트가 정확히 그 모양이다. `jsonb_to_recordset`을 문장마다 다시 편다.

### 처음으로 pgTAP을 실제로 돌렸다

(7)에서 "Docker가 없어서 0149~0157이 한 번도 실행되지 않았다"고 적어둔 그
pgTAP이다. 이번 세션 컨테이너에는 로컬 PostgreSQL 16과 `postgresql-16-pgtap`이
있어서 **`91_hive_formation_test.sql`을 31 단언 전부 초록으로 돌렸다.**

**단, 스텁 스키마 위에서다.** 마이그레이션 체인 전체가 아니라
`servers`/`players`/`app_users`/`current_app_role`/`has_permission` 등을 실제
정의대로 흉내 낸 최소 스키마 위에 0165~0167만 얹었다. 그래서 이 파일의 문법과
단언은 검증됐지만, **다른 마이그레이션과의 상호작용은 여전히 미검증**이다.
Docker 있는 머신에서 `supabase test db`는 그대로 남은 숙제다.

돌리는 법(재현):

```
su postgres -c "PATH=/usr/lib/postgresql/16/bin:\$PATH initdb -D <dir>/data -U postgres -A trust"
# 스텁 + 0165~0167 적용 후
su postgres -c "PGOPTIONS='-c search_path=public,extensions' psql ... -f supabase/tests/91_hive_formation_test.sql"
```

pgTAP은 `extensions` 스키마에 들어가므로 `search_path`에 넣어야 `plan()`이
보인다. 그거 하나로 31개가 전부 "function plan(integer) does not exist"였다.

### 프론트

- `lib/hiveFormation.ts` — 순수 기하 + 자동 배정. 21개 테스트.
- `features/hive/TileGrid.tsx` — **타일 하나하나가 엘리먼트가 아니다.** 45칸
  창이면 2천 개고, 팬 한 번이 2천 노드 리렌더가 된다. 격자는 배경
  그라디언트이고 엘리먼트는 정보를 나르는 것(베이스·앵커·목격)뿐. 그래서
  클릭은 엘리먼트가 아니라 **투영으로** 좌표가 되고, y 뒤집기가 그 안에 있다.
  6개 테스트.
- `features/hive/HivePage.tsx` — 맨 위가 **"내 타일: 509, 391"** 한 줄이다.
  84명한테 필요한 건 포메이션이 아니라 자기 칸 하나다. 라이브가 아닌 초안을
  보고 있으면 그렇게 말한다.
- `features/hive/FormationEditor.tsx` — 그리기 → 저장 → 배정. **이 순서는
  강제된다**: 타일은 저장되기 전엔 id가 없고 배정은 id로 슬롯을 가리킨다.

**브라우저로 실제 확인함**(로컬 룩어라운드 빌드 + Playwright): 빈 땅 클릭 →
배치, 두 칸 옆 클릭 → `"515, 388에 있는 베이스와 땅이 겹칩니다"` 거부,
베이스 클릭 → 제거. 스크린샷으로 3칸 피치가 배경 격자와 정확히 맞는 것도 봤다.

### 아직 안 된 것 · 결정 안 한 것

- **`hive.plan`은 officer·admin에게만 켜져 있다.** 멤버가 빈 칸을 스스로
  집는 건 안 만들었다 — 정책이 복잡해지고, 요청은 "배정해서 알려주기"였다.
- 슬롯 순번(`ordinal`)은 **에디터가 항상 읽기 순서로 다시 매긴다.** 컬럼은
  기획자가 정한 순서를 담을 수 있지만 그걸 입력할 UI가 없다.
- 목격 타일(`world_cities_in_box`)은 **참고일 뿐 차단하지 않는다.** 파괴되거나
  실드 풀린 베이스는 랜덤 텔포되므로 맵 탭의 경고가 그대로 적용된다.
- Discord 알림 연동 안 함. 지금은 "목록 복사" 버튼으로 붙여넣는다.
- 포메이션은 서버당 하나만 라이브. 이건 unique index로 강제된다.
...
> **이 명령은 이제 쓰지 말 것 — (12)를 읽는다.** 여기 남겨두는 건 기록이라서지
> 따라 하라는 뜻이 아니다. `--linked`가 내는 `__InternalSupabase` 블록이 CI의
> `--local` diff 검사에 걸리고, 그게 나중에 하루를 잡아먹었다. 지금은
> `pnpm db:types`가 `--linked` 모양을 아예 거부한다.
