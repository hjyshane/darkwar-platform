...
작성 2026-08-01, 갱신 2026-08-27. 다음 세션이 이 문서만 읽고 이어받을 수 있게 쓴다.

> **다음 세션은 바로 아래 「2026-08-27 상태 (7)」부터 읽는다.** 그 아래는 배경이고,
> 일부는 이미 낡았다.

---

## 2026-08-27 상태 (7) — 랭킹 미니멈, 주간 탭, 빌딩별 기준선 (0155~0157 + 프론트, PR #244·#245)

(6)의 성능 작업이 끝난 뒤 같은 날 들어온 기능 요청 넷과 후속 하나. 전부
프로덕션 적용·머지 완료.

### 요청 1 — 등급 리포트가 "바뀐 사람"만 보여줬다

`RankReportSetting`은 tier가 움직인 멤버만 나열했다. 그런데 tier 변화는
**백분위 경계를 넘었느냐**라서, 15점이 올라도 경계를 못 넘으면 "변화 없음"으로
읽힌다 — 대부분의 인원이 대부분의 기간에 그렇다. `Rank changes` / `Everyone`
두 버튼을 달고, Everyone은 점수 내림차순 + 이전 기간 대비 델타를 같이 보여준다.
양쪽 중 하나라도 null이면 델타는 "—" — 이전 기간에 점수가 없던 사람은 제자리에
있었던 게 아니라 측정이 안 된 것이다.

### 요청 2 — 최소 기준선과 한 단계 강등 (0155·0156)

점수는 **순수하게 상대적**이었다(연맹 내 백분위 × 가중치). "누가 누구보다
많이 했나"는 답하지만 "이 사람이 충분히 했나"는 못 답한다. 조용한 2주에는
바닥권도 R2가 된다. 연맹이 소리내어 말하는 하한선이 있는데 보드가 그걸 말할
방법이 없었다.

- 설정 위치: `rank_tiers` 안에 가중치와 나란히.
  `"minimums": { "enabled": false, "donation_weekly": 0, "duel_weekly": 0 }`
- 미달이면 **한 단계**: R3→R2, R2→R1. **R1은 R1로 남는다** — 게임에 그 아래
  등급이 없고, 없는 등급을 만들면 보드에서만 존재하는 랭크가 된다. R1에게
  벌칙은 빨간 표시와 `minimum_missed`가 전부.
- **주간만, 일간은 뺐다.** 일간 보드(`daily_donation`,
  `alliance_battle_daily`)도 있고 요청도 일간을 포함했지만, 일간 수치는
  **수집기가 그날 돌았는지**만큼만 존재한다. 화요일에 한 번도 캡처 안 된 멤버와
  화요일에 아무것도 안 한 멤버가 구분되지 않는다 — 이 저장소의 상시 규칙이
  "부재는 0이 아니다"이고, 일간 하한선은 **우리 수집 구멍으로 사람을 강등**시킨다.
  사용자가 이 근거를 보고 주간만으로 결정했다.
- 같은 이유로 **읽히지 않은 주는 미달이 아니다.** 진행 중 2주차는 아직 일어나지
  않았고, 수집기가 놓친 주는 아무 증거도 아니다.
- 오피서(unranked)·신입(unmeasured)은 플래그만 계산되고 강등은 없다 — 0072의
  "측정하되 등급 매기지 않는다"는 그대로.
- **scoring_version 4 → 5.** 저장소 규칙(점수 규칙 변경은 덮어쓰지 않는다)이고,
  대가가 있다: `rank_period_movement`는 **같은 버전끼리** 비교하므로 v5로 두
  기간이 쌓이기 전까지 이동 화살표와 델타가 비어 있다. 고장이 아니라 정답 —
  이 규칙으로 채점된 이전 기간이 존재하지 않는다.

0156이 플래그를 멤버 목록까지 가져온다. **라이브 조인이 아니라 0106의 사전계산
행에 태워서**: `player_current_rank`는 전 기간 × 전 멤버 DISTINCT ON이고,
멤버마다 그걸 조인하는 게 정확히 0107이 되돌려야 했던 모양이다. refresh가
이미 그 뷰를 한 번 읽으므로 컬럼 하나는 공짜다.

### 함정 — `select *` 뷰의 별은 생성 시점에 한 번 펼쳐진다

0156 첫 push가 `column latest.below_minimum does not exist`로 실패했다.
`rank_period_latest`가 `select distinct on (...) *`로 쓰여 있는데, **뷰의
별표는 만들 때 한 번 확장되고 그대로 굳는다.** 0155가 테이블에 추가한 컬럼이
그 뷰에는 없고, 그 위의 모든 독자가 그걸 물려받는다. `create or replace`로
다시 만들면 별이 재확장되면서 새 컬럼이 **뒤에** 붙는다(그래서 기존 계약도
안 깨진다).

컬럼을 추가하면 **그 테이블을 `*`로 읽는 뷰를 전부 다시 만들어야 한다.** 이건
어제 배운 "뷰의 현재 정의는 마지막 마이그레이션"과 같은 계열의 다른 얼굴이다.

### 요청 4 — 멤버 탭을 두 보드로 (0157)

0134는 멤버 탭을 **마지막으로 끝난 2주**에 고정해 뒀다. 진행 중 기간은 주간
읽기가 하나뿐이라 점수가 반쪽이기 때문이고, 그 결정은 유지했다. 대신 **지금
살고 있는 2주**를 볼 방법이 없다는 게 문제였다 — "이번 주 나 선 넘었나"는
수요일에 하는 질문이다.

`member_current_period_contribution`(0157): 현재 로스터 멤버별로 진행 중
기간의 **주간 원본 읽기값** — 백분위도, 가중치도, tier도 없다. 여기의 어떤
숫자도 랭크를 못 움직이므로 미완성 기간을 보여줘도 안전하다.
`rank_period_snapshots`를 읽지 **않는** 이유: 그 테이블은 누군가 Build를 누른
기간에만 행이 있고, 화면이 "이번 주에 오피서가 버튼을 눌렀는가"에 의존하면 안
된다. 프론트는 `Last ranking` / `This ranking · weekly` 탭 + Week 1 / Week 2 /
Both 선택.

### 요청 3과 후속 — 빌딩별 기준선, 그리고 안 보이던 `!` (PR #245, 마이그레이션 없음)

시즌 건물 경고는 **이미 있었다**(`season_building_alert` + `isBehind`,
`app_settings`는 `public_read`라 누구나 본다). 빠진 건 기능이 아니라 스위치가
꺼져 있다는 것뿐이었다.

이어서 사용자가 "전체가 아니라 **빌딩 종류별로** 레벨"을 요청했다. 0158은
없다 — 값이 jsonb라 `perBuilding` 키가 마이그레이션 없이 자랐다(그 파일의
옛 주석이 이 경로를 미리 적어 뒀다). **레벨이 안 적힌 빌딩은 아예 판정하지
않는다** — "이번 주엔 랩이 중요하다"를 말하는 방법이 그것이다. 옛 단일 레벨은
그대로 의미를 유지(카탈로그 전체에 적용)하되, 빌딩별 값을 하나라도 저장하면
은퇴한다. 섞으면 일부러 비워둔 빌딩에 바닥이 깔린다.

**`!`가 안 보였던 진짜 이유: `.behind-mark`에 CSS가 아예 없었다.** 이름 열에
맨텍스트 `!` 하나는 구두점으로 읽힌다. 지금은 빨간 채움 배지(글리프는 배지
안에 유지 — 색은 강조지 메시지가 아니다), 그리고 **문제의 숫자 자체**도 자기
열에서 표시된다. 7열짜리 행에서 이름만 표시하면 어느 빌딩이 걸렸는지 눈으로
찾아야 했다. 툴팁은 "10 미만"이 아니라 빌딩 이름과 기준 레벨을 말한다.

### 프로덕션 실측

| | |
|---|---|
| 0155~0157 적용 후 `member_roster` | **바이트 동일** (82행, 등급·점수 불변) |
| 로스터 읽기 | 0.126s |
| 새 주간 뷰 (82명, 1인 4프로브) | 0.40s |
| 미달 표시된 인원 | 0 — 미니멈은 꺼진 채로 배포 |

타입은 `supabase gen types typescript --linked --workdir C:\darkwar-platform`로
재생성했다(로컬 스택이 없으므로 `--local` 불가). 오늘 추가된 뷰들이 통째로
빠져 있었어서 695줄이 들어왔다.

### 남은 것 · 아직 검증 안 된 것

- **pgTAP은 오늘 하루 한 번도 안 돌았다.** Docker가 없다. 0149~0157 일곱 개와
  새 단언 26건(36·42·48·85·89)이 **한 번도 실행되지 않았다.** Docker 있는
  머신에서 `supabase test db`부터.
- **미니멈은 숫자를 넣고 Rebuild를 눌러야 효력이 생긴다.** 설정만 저장하면
  아무 tier도 안 움직인다.
- **v5 이동 표시는 두 번째 v5 기간이 쌓여야 돌아온다.** 위의 이유.
- 브라우저 확인은 사용자가 했다(배지 잘 보임). 로컬 빌드는
  `127.0.0.1:54321`을 가리키므로 여기서 띄워봐야 아무것도 증명 못 한다.
- `alliance_roster_history` 0.99s가 트렌드 탭에서 가장 느린 것으로 남아 있다.

---

## 2026-08-27 상태 (6) — 읽기 시점 계산을 다시 걷어냈다 (0149~0154, PR #239·#240·#241·#242)

사용자가 시즌 건물 보드에서 `canceling statement due to statement timeout`을
보고했다. 파고들어 보니 **한 화면의 문제가 아니라 네 화면의 같은 병**이었고,
그중 셋은 아무도 보고하지 않은 채 느렸다. 마이그레이션 여섯 개, 전부 프로덕션
적용·머지 완료.

**이 절의 모든 수치는 `service_role`로 REST를 직접 때려 잰 것이다** (RLS 우회,
따라서 순수 쿼리 비용). `.env`의 `SUPABASE_URL` + `SUPABASE_SECRET_KEY`로
`curl -w '%{time_total}'`. 멤버 세션은 재지 못했다 — 아래 「못 잰 것」 참고.

### 한 줄 요약

| 화면 | 뷰 | 전 | 후 |
|---|---|---|---|
| 시즌 건물 보드 | `member_season_buildings_by_member` | 5.60s (멤버는 타임아웃) | **0.129s** |
| **오버뷰(첫 화면)** | `alliance_roster_latest` `.in()` | 6.05s | **0.29s** |
| 연맹 페이지 | `alliance_roster_latest` `.eq()` | 0.30s | 0.13s |
| 로스터 탈퇴자 | `alliance_departures` | 4.19s | **0.18s** |
| 트렌드 탭 | `alliance_power_history` | 4.33s | **0.16s** |
| 트렌드 탭 | `alliance_daily_contribution` | 6.38s (필터 없으면 500) | 0.67s |

### 0149 — 보드가 1.76M행 대신 82행 테이블에 묻는다 (#239)

`season_building_snapshots`는 24,349행뿐인데 `member_season_buildings`가
**LIMIT 1에 3.2초**였다. 비용은 건물이 아니라 0146이 붙인 조인이었다:
`alliance_roster_latest`가 `alliance_member_snapshots` **1,757,827행**을 두 번
집계한다 — 그룹 전체 256개 연맹에 대해, 이 보드가 쓰는 82명을 찾으려고.
**조인의 등가조건은 뷰의 집계 밑으로 안 밀린다**(0103이 363k행에서 이미 적어둔
것, 지금은 다섯 배).

`member_roster_current`(0106)로 교체했다. 멤버 표가 읽는 바로 그 82행이라
0146의 논지("두 화면이 '누가 연맹원이냐'에 다르게 답하면 안 된다")가 한 걸음
더 간 셈이다. 바꾸기 전 프로덕션에서 양쪽 다 82행임을 확인 → 내용 불변.
DISTINCT ON 키에 맞는 인덱스 `(player_id, building_type_id, captured_at desc)`도
추가(0138의 것은 두 번째 컬럼이 이 쿼리의 세 번째라 못 쓴다).

### 0150·0151 — `eq`와 `in`이 스무 배 차이 났다 (#240)

**같은 연맹, 같은 뷰.** `alliance_id=eq.<x>` 0.30초, `alliance_id=in.(<같은 x>)`
**6.05초**. `eq`는 플래너가 `group by alliance_id` 밑으로 미는 qual이 되고,
`in`은 `= any (array[...])`가 되어 **안 밀린다**. 그래서 256개 연맹 전부를
집계한 뒤 하나만 남긴다.

**오버뷰 패널이 `.in()`을 쓴다**(자기 연맹이 여럿일 수 있어서). 즉 첫 화면이
적용됐다고 믿은 필터 값으로 6초를 내고 있었다.

셋 다 **연맹 주도(alliance-driven)**로 바꿨다. 전체 테이블 CTE →
`alliances`에 상관된 LATERAL. 필터가 PK에 떨어지고 그 밑은
`alliance_member_snapshots_alliance_captured_idx`(0003) /
`alliance_contribution_snapshots_uid_type_idx`(0110) 인덱스 하강.

의미는 일부러 그대로 뒀다:
- `snapshot_complete`은 여전히 최신 배치를 `member_count`와 견주고, 여전히
  거르지 않고 표시만 한다(0067의 "덜 스크롤된 캡처" 함정).
- 트렌드의 연맹원 정의는 **"한 번이라도 본 사람"**이지 현재 로스터가 아니다.
  차트는 이력이다 — 오늘 로스터로 7월을 재귀속하면 누가 나갈 때마다 과거가
  바뀐다. 42에 "탈퇴자의 기부는 그날에 남는다" 테스트를 박아 뒀다.

### 0152 — **0150이 서비스를 잠갔다. 내 실수다** (#240에 포함)

0150을 **0067 본문에서** 재작성했다. 그런데 그 두 뷰가 마지막으로 정의된 곳은
0067이 아니라 **0077**이고, 0077이 predicate에 `or public.is_service_request()`를
더해 놨었다. 옛 본문에서 다시 쓰면서 그게 조용히 되돌아갔다.

**push 전후 프로덕션 출력 diff가 잡았다.** 로스터 82행·탈퇴 16행이 0행이 됐고,
같은 배치에서 고친 `alliance_daily_contribution`은 — 그건 0077에서 읽었으므로 —
바이트 동일하게 돌아왔다. 0152가 disjunct 하나를 되돌린다.

> **교훈: 뷰의 현재 정의는 그것을 소개한 마이그레이션이 아니라 마지막으로
> 건드린 마이그레이션이다.** 고치기 전에 `supabase/migrations/` 전체를
> `view public.<이름>`으로 grep해서 **마지막** 것을 본다. CLAUDE.md의
> "컬럼을 바꾸면 저장소 전체를 grep한다"와 같은 병의 다른 발현.

### 0153 — 윈도우 함수가 88,079행을 매번 정렬하고 있었다 (#241)

트렌드 탭은 쿼리 3개를 병렬로 쏘는데, 탭이 기다리는 건 0151이 고친 0.67초짜리가
아니라 **`alliance_power_history` 4.33초**였다. 0151은 트렌드의 엉뚱한 3분의 1을
고친 것이다.

원인: 윈도우 함수 둘 다 `partition by s.observation_id`. **qual은 모든 윈도우의
파티션 컬럼을 참조할 때만 WindowAgg 밑으로 밀 수 있다.** `alliance_id = $1`은
`observation_id`를 언급하지 않는다 → `alliance_snapshots` **88,079행 전부**를
정렬·윈도우한 뒤 한 연맹만 남긴다. 이 뷰를 읽는 두 화면(트렌드·비교) 다
연맹으로 거른다.

거꾸로 계산한다: 먼저 거르고, 살아남은 행마다 **그 관측**이 몇 행이고 몇 서버를
덮었는지 LATERAL로 묻는다(`observation_id`는 0128이 이미 인덱스). `board_scope`
유도식은 0081 그대로 — 서버 보드와 그룹 보드는 랭크 숫자 말고 페이로드가 동일해서
`min(server_id) <> max(server_id)`만이 둘을 가른다.

48에 함정 하나를 박았다: **`captured_at`은 같고 `observation_id`만 다른 두 관측**.
`captured_at`으로 키를 잡으면(이걸 잘못 쓰는 가장 그럴듯한 방법이다) 네 행이 한
보드로 접히고 아카이브의 모든 서버 보드가 조용히 cross_server가 된다.

### 0154 — 시즌 보드는 읽을 때 계산하기를 그만뒀다 (#242)

0149로 5.6초 → 0.19초(service_role)가 됐는데 **멤버 세션은 여전히 타임아웃**이라고
사용자가 보고했다. 밖에서 재는 모든 값이 "빠르다"고 말하고, 정작 중요한 경로는
여기서 잴 수 없다.

**같은 자리에 온 네 번째 화면이다**(0106 멤버 표, 0107 연맹 랭킹, 그리고 이것).
마이크로 인스턴스 + 행별 RLS + 붕괴한 추정치 → 플래너가 내부 쿼리를 멤버마다
다시 도는 모양을 고른다(0105가 92회 재계산을 실측했다). 매번 처방이 같았고 매번
들었다. **못 보는 계획을 진단하는 대신 플래너의 선택지를 없앴다.**

`player_season_buildings_current`: **플레이어당** 1행(멤버당이 아니다), 건물 타입별
최신 레벨을 jsonb로 접어서. 멤버십은 뷰에서 `member_roster_current`와 라이브
조인 — 그래서 테이블은 `season_building_snapshots`의 순수 함수이고, 탈퇴는 prune
없이 떨어져 나간다.

**리프레시는 0106과 달리 증분이다.** 로스터 배치는 몇 분에 한 문장이지만 맵
스윕은 insert 문장이 수백 개 연속으로 들어온다. statement 트리거가 전이
테이블에서 player_id를 뽑아 **그 플레이어만** 스냅샷에서 재계산한다 — 도착한 행을
저장된 jsonb에 **머지하지 않는다**. 0138이 레벨의 단조성을 확인해 뒀으므로 낮은
값은 항상 더 오래된 관측이고, 머지식이면 백필 한 번이 보드 전체를 아래로
끌어내린다(85에 이 테스트가 있다).

프로덕션 실측: 보드 출력 **바이트 동일**(82행), 읽기 **0.129초**, 테이블 704명,
**타겟 리프레시 0.136초 / 전체 리프레시 0.172초** — 둘 다 네트워크 바닥이라
수집기 쓰기가 무는 비용은 잴 가치가 없다. 전체 리프레시 후 보드 다시 동일(prune
경로 확인). 뷰 이름·컬럼·순서 그대로 → 대시보드 무변경, 재배포 불필요.

### 검증 방식 — 이번에 이걸로 버텼다

**프로덕션 출력을 push 전에 떠 놓고 push 후에 diff.** 서비스 키 + REST로 뽑아
스크래치에 JSON으로 저장하고 `diff -q`. 결과: 자기 연맹 로스터 82행 / 탈퇴 16행 /
기여 82행 / power_history 816행(cross_server 541·server 275, board_size 1~100) /
보드 82행 — **전부 바이트 동일**, 전체 로스터 행 수 7,431 불변.

0152를 잡은 게 이 diff다. pgTAP을 못 돌리는 동안 이게 유일한 실증 장치였다.

### 못 잰 것 · 남은 것

- **pgTAP을 하루 종일 한 번도 못 돌렸다.** 이 머신엔 Docker가 없다(메모리:
  vmmemWSL 5.5GB 건으로 제거됨). 0149~0154 여섯 개와 새 단언 16건이 **한 번도
  실행되지 않았다.** Docker 있는 머신에서 `supabase test db`를 먼저 돌릴 것.
- **멤버 세션을 못 쟀다.** service_role은 RLS를 우회하고, Chrome 확장은 이 세션에
  안 붙어 있고, 세션 토큰을 빌리지는 않았다. 0154는 **관측하지 못한 실패를 보고**
  친 것이다 — 근거는 같은 서명의 선례 셋(0105·0106·0107). 보드가 여전히
  타임아웃이면 진단이 틀린 것이고, 다음 수는 또 다른 추측이 아니라 대시보드 SQL
  에디터의 EXPLAIN이다:
  ```sql
  select set_config('request.jwt.claims', json_build_object('sub', (select user_id from public.app_users where role = 'member' limit 1), 'role', 'authenticated')::text, false);
  set role authenticated;
  explain (analyze, buffers) select * from public.member_season_buildings_by_member limit 500;
  reset role;
  ```
- `alliance_roster_history` **0.99초**가 이제 트렌드에서 가장 느리다. 안 건드렸다.
- 대시보드가 읽는 나머지 20개 뷰는 0.12~0.87초로 전부 정상.

### 작업 방식 — 이번에 배운 것

**워크트리의 마이그레이션은 `db push`에 안 보인다.** `--workdir C:\darkwar-platform`
는 **메인 체크아웃의** `supabase/migrations/`를 읽는다. 브랜치에만 있는 파일은
없는 것과 같고, push는 "up to date"라고 말한다(0108 때 한 번 당했다). 급하면
**그 파일만 메인 체크아웃에 복사 → push → 삭제**. 머지 후엔 메인 체크아웃에서
`git pull`. 이걸 안 하면 다음 push가
`LegacyDbPushMissingLocalError`(원격에만 있는 버전)로 막힌다 — 이번에 그렇게 막혔다.

**push 로그의 Docker 에러 벽은 무시해도 된다.** 마이그레이션 카탈로그 캐시일
뿐이고, 판단은 마지막 줄 `"message":"Finished supabase db push."`로 한다.

---

## 2026-08-21 상태 (5) — 맵이 풀렸다. 시즌 건물은 아직

브랜치 `season-3-dashboard-ranking-2cb1d7` (머지 후에도 계속 씀). 시즌 랭킹
보드는 **프로덕션에 배포 완료**, 데이터도 들어간다.

### 새 캡처 없이 맵을 해독했다

`C:\DW_data\live.db`에 이미 `world.get.new` **8,016개**,
`world.get.detail.new` **787개**가 쌓여 있었다. 타일 41,298개. 전부 여기서
읽었다. **가진 것부터 다시 읽는 게 두 번째로 먹혔다**(첫 번째는 아레나 `stage`).

- `pointId`가 곧 좌표다. `x * 1000 + y`. `point == pointId` 543/543,
  y가 1000 이상인 샘플 0개, 모든 뷰포트가 `maxAreaSize` 1000.
- **type 3 = 플레이어 도시**. uid(`f3.1`), **이름**(`f3.14`, 상세 응답의
  `name`과 288/294 일치 — 예전에 연맹 태그라고 적어둔 건 틀렸다),
  **본부 레벨**(`f3.4`, 로스터 `hq_level`과 정확히 349/394 일치, 나머지는 ±1~2로
  캡처 시점 차이).
- 그래서 **팬만 해도 "이 멤버 어디 있고 본부 몇 렙인지"가 나온다.** 오브젝트를
  열 필요가 없다.

파이프라인은 다 붙였다: `protocol/worldmap.py`(디코더),
`normalize/world_map.py`(파서), 0137 `world_city_snapshots`(스키마),
pgTAP 83, 픽스처. 로컬 라운드트립 134타일 failed=0, player 134/134 해결.

### type 21은 시즌 건물이 아니다 — 두 번 반증했다

8,430타일에 소유자 uid가 붙어 있고 스펙 끝 두 자리가 1~4라 레벨처럼 보인다.
CBFW 것만 2,542개, 소유자 93명. **그런데 레벨이 아니다.**

1. 오브젝트 id 기준으로 스펙이 **한 번도 안 변했다** (재관측 19,983개 중 0).
2. 좌표 기준 + 소유자 고정 시 전이가 **대칭**이었다 (증가 457 / 감소 396).
   업그레이드면 단조증가여야 한다.
3. 게다가 563개는 **움직인다.**

`vl`도 레벨이 아니다 — 507개 샘플 전부 0.

### 시즌 건물 레벨(연맹원별)은 여전히 미해결

레벨을 가진 건 `alBuilding`(type 15) 하나인데 **연맹 단위**다. 3개 센터
(41000/42000/43000), 전부 level 1, 개인별 분해 없음. 787개 상세 응답 중
`alBuilding`은 4개뿐이었다.

**다음에 할 일 하나**: 캡처 켜고 **멤버 소유 시즌 건물을 맵에서 하나 열어라.**
그 타일의 `f2`가 뭔지만 알면 끝난다. 안 열어본 타입: 4, 13, 14, 11, 22, 23,
25, 29.

### 이번에 밟은 함정 두 개 (둘 다 내 실수, 기록해 둔다)

- **포인트 키를 `46b`로 적었는데 실제는 `b64`다.** 분석 스크립트가 키들의
  *문자 집합*을 출력한 걸(`{'4','6','b'}` 정렬) 리터럴 값으로 읽었다.
- **포인트는 와이어에서 `bytes`다.** `b64:` 접두사는 이 프로젝트의
  `models.json_default`가 붙이는 것이지 게임이 보내는 게 아니다. 같은 바이트가
  **세 가지로** 직렬화된다: 와이어=bytes, 저널=`b64:`+표준 base64,
  픽스처=`model_dump_json`의 **base64url, 접두사 없음**. 셋 다 받도록 고쳤고
  세 형태가 같은 타일로 디코드되는지 테스트가 박혀 있다.

### 로컬 상태

- **0137은 2026-08-21 프로덕션에 적용됐다**(dry-run으로 대상이 0137 하나임을
  먼저 확인). 원격 마이그레이션 목록이 0137까지 온다.
- **그래서 또 반대 방향 drift가 있다.** 원격엔 0137이 있고 `main`엔 파일이
  없다 — 해당 커밋(`62ba73d`)이 아직 머지 전이기 때문이다. 머지하면 해소된다.
  0136 때와 같은 상황이고, 대시보드가 이 테이블을 아직 안 읽으므로 사용자에게
  보이는 영향은 없다.
- `renormalize`로 저널의 맵 관측을 되돌리면 실제 맵 이력이 대량으로 들어온다.
  단, 아웃박스가 통째로 pending이 되므로 **시즌 보드 때처럼 event_type으로
  좁혀서** 보낼 것.
...
- **0136은 2026-08-20 프로덕션에 적용됐다**(`supabase db push`, dry-run으로
  적용 대상이 0136 하나임을 먼저 확인). 원격 마이그레이션 목록이 0136까지 온다.
  **머지 전에 넣은 이유**: 대시보드가 머지되면 Cloudflare가 1분 안에 배포하는데,
  Season 3는 CBFW 옆 최상위 탭이라 테이블이 없으면 94명 전원에게 에러로 보인다.
- 그래서 **머지 전까지는 반대 방향 drift가 있다.** 원격에는 0136이 있고
  `main`에는 파일이 없다. 머지되면 해소된다. 그 사이 `main`에서
  `supabase migration list`를 보면 remote에만 0136이 있는 것으로 나온다.
- 워크트리는 Supabase CLI에 링크돼 있지 않다(링크 상태는
  `C:\darkwar-platform\supabase\.temp`). 위 push는 그 파일들을 워크트리로
  잠깐 복사해서 돌리고 **바로 지웠다** — 워크트리에서 실수로 프로덕션을 건드리는
  일이 없도록 원래 상태로 되돌린 것이다. 다시 필요하면 같은 방법으로 복사한다.
- 로컬 스택에는 `psql`로 직접 넣어 둔 0136이 그대로 있다.
