...
작성 2026-08-01, 갱신 2026-08-04. 다음 세션이 이 문서만 읽고 이어받을 수 있게 쓴다.

> **다음 세션은 바로 아래 「2026-08-04 상태」부터 읽는다.** 그 아래는 배경이고,
> 일부는 이미 낡았다.
...
`DarkWar-Ingest`(`ingest-dir` 60초 주기), `DarkWar-Sync`. 등록 스크립트는
...
**1. routine을 지금 짜면 돌아가지 않는다.** `dw-ui-worker`는 탭 후 **20초**
...
**3. 보존 정책 — 정의부터 정해야 한다.** 사용자 요청은 "우리 연맹/플레이어는
...
- **동기화 배지가 하트비트만 보고 있었다** — 위 「막힌 것 2」가 화면에 안 드러나던
  이유다. 배지는 "뭐라도 들어오고 있나"에 답한다고 주석에 적어놓고 실제로는
  `dw-sync`가 자기 자신에 대해 찍는 하트비트만 읽었다. 멈춤이 두 번 다 초록불
  뒤에서 일어났다. 이제 **수집기가 채운 최신 알림 시각을 같이** 보여준다
  (`● Real-time sync · data 5h ago`). 관리자가 타이핑하는 토픽(영웅·공지·설정)은
  세지 않는다 — 그걸 세면 같은 거짓말이 한 테이블 뒤로 옮겨갈 뿐이다.
  **임계값은 일부러 안 붙였다**: 순회가 없는 지금은 몇 시간 공백이 정상이라
  경보로 만들면 종일 빨간불이 되어 정작 진짜일 때 안 읽힌다. 순회가 붙은 뒤
  이미 화면에 있는 그 숫자 위에 임계값을 얹으면 된다
...
**~~남은 진짜 항목~~ — 이것도 닫혔다 (2026-08-04 확인).** 듀얼 일간·주간은 상대
연맹 선수도 포함한다(165 = 우리 93 + 상대 72). 상대 72명도 `player_contributions`
행을 갖고, 값 자체는 틀리지 않다(그 사람의 실제 점수다). 이 항목은 "연맹 합계를
내는 화면을 만들 때 처리하면 된다"로 남아 있었는데, **그 화면은 이미 만들어졌고
이미 처리하고 있다** — `OverviewPanel`이 `is_own` 연맹의 `current_alliance_id`로
멤버 id 집합을 먼저 만들고 기여도 행을 거기에 걸러서(`ours`) 합산한다. 상대 72명은
합계에 들어가지 않는다.

읽는 김에 알아둘 것: 같은 파일의 합계는 **행이 0건이면 0이 아니라 `null`**을
낸다. RLS가 로그아웃 독자에게 행을 한 건도 주지 않는데 거기에 `0`을 찍으면
"아무도 기부하지 않았다"는 없는 근거의 주장이 된다.
