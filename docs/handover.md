...
> **다음 세션은 바로 아래 「2026-09-09 상태 (10)」부터 읽는다.** 그 아래는 배경이고,
> 일부는 이미 낡았다.
...
(8)에서 만든 하이브 포메이션 위에 세 덩어리를 얹었다. 브랜치는
`claude/hive-formation-tile-assignment-qpopfc`, 마이그레이션 **0169~0172**.
**아직 머지 전이고, 머지 뒤에 `supabase db push`가 필요하다.**
...
- **`supabase db push`** — 0169~0172. (8)에서 실패했던 건 `--workdir` 문제였고,
  main 체크아웃을 pull하면 된다.
- **타입 생성이 `--local`과 `--linked`로 갈린다.** CI는 `--local`이라
  `__InternalSupabase` 블록이 없어야 하고, 저장소 스크립트는 `--linked`다. 다음
  `--linked` 재생성이 `db` 잡을 다시 깨뜨린다. 아직 정하지 않았다.
