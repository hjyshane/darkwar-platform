...
- 일간 기부: `vina bao phan 14500` · `baby nur 11980` · `dang 11980`
- 주간 기부: `bored101 86400` · `vina an coup 80820` · `R3hab 90640`

지금은 `get.daily.alliance.donate.rank` 하나만 파서가 있다. **주간이 별도
커맨드인지, 같은 커맨드의 다른 필드인지**를 pcap에서 확인하면 된다. 방법은
`al.battle.rank.info` 때와 같다 — inbound 커맨드를 훑고 상위 3명 점수를 위
숫자와 대조한다.

### 2. 메일 수신 → Discord 계정 연결 (§6.2)

같은 pcap에 **`북경오리구이`가 보낸 메일 수신**이 들어 있다. `push.mail`의
`fromUser`가 보낸 사람 UID로, `contentsArr`에 본문이 차면 확정이고,
`game_identity_links`(일회용 코드를 게임 메일로 받아 UID 매칭)가 열린다.

### 3. 요약에서 상대 연맹 제외

`alliance_battle_daily`/`_weekly`는 **상대 연맹 선수도 포함**한다(165 = 우리 93
+ 상대 72). 지금은 상대 72명도 `player_contributions` 행을 갖는다. 그 자체로
틀린 값은 아니지만(그 사람의 실제 점수다) 멤버 탭이 우리 로스터만 조인해서
화면에는 안 나올 뿐이다. **"우리 연맹"을 스키마가 알아야** 걸 수 있어서 판단이
필요하다 — `alliance_name`은 이미 저장돼 있다.

### 4. 미해석 필드 2개 — 캡처 필요

`docs/capture-backlog.md` **0-b번**. `army`의 `2.9`(승급 조각으로 보임)와
전용무기 숫자. 지난 캡처는 대상 영웅이 전부 최고 성급이라 `2.9`가 아예 안
나왔다. **최고 성급이 아닌 영웅이 있는 편성**을 하나 찍으면 된다.
...
| 6 | 기부 일간·주간 | 일간만 파서 있음. **주간은 캡처 안에 있고 아직 안 봤다** |
...
```powershell
supabase start
supabase db reset          # 0001~0025 전부 적용
...
같은 문서의 **0-b번**. 아레나 화면을 열면서 영웅 5기의 레벨·성급·조각을 적어야
`army`의 남은 두 필드를 판정할 수 있다. 편성이 계속 바뀌므로 화면과 payload가
같은 순간의 것이어야 한다.
