...
```powershell
supabase start
supabase db reset          # 0001~0029 전부 적용
...
**남은 진짜 미검증은 브라우저 렌더링과 Realtime뿐이다.** 데이터 경로 자체는
WSL에서 fixture로 끝까지 돌려봤다 — `dw-collector replay` → `dw-collector sync`
→ Supabase → 패널이 던지는 그대로의 PostgREST 쿼리. 그래서 위 절차에서 실제로
새로 알게 될 것은 `pnpm dev` 이후다.
...
| 확인할 것 | 왜 |
|---|---|
| `Alliance Battle` 열 | **해결됨.** 아래 참조 |
| 기부 두 열 | **데이터 경로는 확인됨** — fixture를 replay→sync해 143행이 두 컬럼에 앉는 것을 봤다. 브라우저로는 아직 |
| 크로스서버 보드 6종 | 목은 `server.rank`와 `hero_power_total`만 흉내 냈다 |
| 아레나 연맹 열 | fixture는 `Alliance01` 같은 살균 placeholder다. 실 값 확인 필요 |
| 100명 규모 테이블 | 목은 6명이었다. 정렬·검색·sticky 열이 실제 길이에서 어떤지. **멤버 탭은 이제 11열이다** — 그중 5개가 기여도(기부 2 + 듀얼 3)다. 폭이 실제로 어떤지가 브라우저에서 처음 드러난다 |
