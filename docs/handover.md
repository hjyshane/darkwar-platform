...
> 기본값(07-20)에 떨어졌고(0090 무렵부터 적혀 있던 그 함정), 두 번째는
> 08-03을 골랐다는데 **쓰기 흔적이 아예 없다**(computed_at 불변, 새 기간
> 행도 없음). SQL엔 조용히 넘어가는 경로가 없으니(성공=무조건 재기록,
> 실패=42501/에러) RPC가 안 나갔거나 화면에 에러가 떴을 것이다. 미해결.
