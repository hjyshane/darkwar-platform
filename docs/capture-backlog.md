...
**남아 있는 것 하나 — `stage`는 아직 어디에도 저장되지 않는다.** 디코더는
`ArmyUnit.stage`로 내보내지만 `arena.py`가 읽지 않고 `arena_entry_heroes`에
컬럼이 없다. 즉 매 관측마다 버려진다. 승격은 이 문서가 아니라 별도 변경이고,
**지금은 안 되어 있다** — `al.battle.rank.info`가 "promote" 판정만 받고 4개월간
글쓴이 없는 컬럼으로 남았던 것과 같은 자리라서 분명히 적어 둔다. 컬럼을 만들
때 필요한 판단(0은 "만렙" 인가 "단계 없음"인가)은 위 표가 이미 답했다: 최고
성급에서는 만렙, 그 아래에서는 단계 0.
